# Batch 0474 — Audit-only tick (99th substantive consecutive)

- **Tick start (UTC):** 2026-05-03T01:03Z
...
- **Tick note:** initial cleanup glob `find .git -name "*.lock" -delete; find .git -name "*.lock.bak" -delete` ran cleanly with carry-forward `Operation not permitted` warning on host-locked `.git/objects/maintenance.lock` and `.git/ORIG_HEAD.lock` (sandbox cannot unlink — same behaviour as b0460..b0473). First `git pull --ff-only` failed with `fatal: bad object refs/remotes/origin/main.lock.bak.b0473_post.20260503T003600Z` — that ref file was zero-length (host-pre-created during b0473 post-push window, after b0473's own b0467_post recovery). Sandbox `unlink` is denied for files under `.git/refs/`, so the recovery path used in b0468 was repeated: copy current `.git/refs/remotes/origin/main` SHA into the bad ref to convert it from a "bad object" into a duplicate-content ref (cosmetic clutter only — git tolerates duplicates). One incidental `test_create_2.txt` ref under `.git/refs/remotes/origin/` (also zero-length, leftover from sandbox unlink-denial probing) was healed the same way. Re-pull then returned `Already up to date.` cleanly. approvals.yaml and BRIEF.md unchanged since 2026-04-30T15:36:40Z (commit b24a938; ~3d 9h at this tick start). gaps.md unchanged since 2026-04-30T21:40:59Z (commit 61d666e). Reparse inventory unchanged from b0473. 3rd substantive tick of UTC date 2026-05-03 (rolls forward from b0473 at 00:32Z).
...
All 89 ZMCC missing candidates remain deferred under v0.3.1-locked reason codes — none addressable by parser_v0.3.1. Empirically re-confirmed across b0375..b0473; verbatim per b0473 audit. ZMSC slice fully ingested across the 24 case HTML files (the additional HTML is the listing index page — not part of the ingestion-target inventory). Inventory exhausted; no fresh fetches initiated this tick.

### Deferred-reason histogram (gaps.md grep, unchanged from b0473)
| Reason code | Count |
|-------------|------:|
...
- 99 consecutive substantive audit-only ticks (b0375..b0383 + b0385..b0474; excludes the failed-pull tick between b0442 and b0443 and the aborted-pull tick at 19:33:50Z between b0464 and b0465).
- UTC date 2026-05-03 (3rd substantive tick of new day; rolls forward from b0473 at 00:32Z).
...
## Recommendation (unchanged from b0441..b0473)
...
Expect another zero-length `.git/refs/remotes/origin/main.lock.bak.b0474_post.<ts>Z` ref to be created by host between this tick's push and the next tick's pull. Recovery is the same one-shot SHA-copy used in b0468/b0474. Recommend the host-side `find .git -name "*.lock*" -delete` extension still pending Peter sign-off.
