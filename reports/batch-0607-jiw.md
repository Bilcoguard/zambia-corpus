# Batch 0607 — Judgment Ingestion Worker (JIW)

**Tick start:** 2026-05-12T07:06Z (UTC) — scheduled run
**Worker:**     judgment-ingestion (separate from main worker / repair worker)
**Phase:**      read-only confirmation — FTS5-blocked (23rd consecutive blocked tick)
**Verdict:**    `tick-complete-fts5-blocked-readonly-no-mutation`
**Read-only confirmation tick:** 4 of 5 (per b0603 escalation guidance, continued by b0605 and b0606)
...
by b0605 and b0606 by re-running only the safe read-only `PRAGMA` and
`SELECT` probes:
...
- `SELECT MIN(rowid), MAX(rowid), COUNT(*) FROM records` → `(1, 1892, 1892)` —
  records table rowids are dense (no gaps); FTS5 rowid sparseness is the
  only divergence, as previously characterised.
- `md5(corpus.sqlite)` → `686f8197193a27b0f979156b833352fa` — **byte-identical** to
  b0598-pre clean backup, b0603, b0605, and b0606.

The FTS5 `INSERT … VALUES('integrity-check')` probe used in b0603 was
**deliberately omitted this tick** (continuing b0605/b0606 posture) — it is
...
- `find .git -name "*.lock.bak" -delete` → ok
- `git pull --ff-only` → `Already up to date` (HEAD `c6edd47` — repair-worker
  STOP entry pushed after b48d60f / b0606)
  - Pre-existing virtiofs/fuse mount quirk continues (`Operation not
    permitted` on certain stale `.git/objects/maintenance.lock` unlinks);
    same pattern as b0598+; not a blocker (pull completes cleanly).
...
Per the priority order in the task file and the b0603/b0605/b0606
standing guidance:
...
Executed via Python `sqlite3` opened with the URI flag `mode=ro` to
guarantee no journal can be created. Probe script run inline; no
derivative script committed.
...
SELECT MIN(rowid), MAX(rowid), COUNT(*) FROM records
                   → (1, 1892, 1892)         (0 rowid gaps — dense)
```

**Delta from b0606:** none. State is byte-identical.
...
This is the **4th of 5** consecutive read-only confirmation ticks per
b0603's escalation guidance. **One** more such tick (b0608 = 5-of-5)
without operator action and JIW will append a "JIW completion suggested
— awaiting human sign-off" line to worker.log (does **NOT** flip
approvals.yaml `complete: true`).
...
until operator performs the records-table dump-and-rebuild described in
the b0603 report. After **one** more such tick (b0608 = 5-of-5), JIW
will append a "completion suggested — awaiting human sign-off" line to
worker.log per the escalation protocol.

## Operator escalation (12th)
