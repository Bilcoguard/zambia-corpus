# Batch 0476 — Audit-only tick (101st substantive consecutive)

- **Tick start (UTC):** 2026-05-03T02:33Z
...
- **Tick note:** initial cleanup glob `find .git -name "*.lock" -delete; find .git -name "*.lock.bak" -delete` ran cleanly with carry-forward `Operation not permitted` on host-locked `.git/objects/maintenance.lock` (sandbox cannot unlink — same behaviour as b0460..b0475). First `git pull --ff-only` failed with `fatal: bad object refs/remotes/origin/main.lock.bak.b0475_post3.20260503T013905Z` — that ref file was zero-length (host-pre-created during b0475 post-push window). Sandbox `unlink` is denied for files under `.git/refs/`, so the recovery path used in b0468/b0474/b0475 was repeated: copy current `.git/refs/remotes/origin/main` SHA (`f937e42`) into the bad ref to convert it from a "bad object" into a duplicate-content ref (cosmetic clutter only — git tolerates duplicates). Re-pull then returned `Already up to date.` cleanly. approvals.yaml and BRIEF.md unchanged since 2026-04-30T15:36:40Z (commit b24a938; ~3d 11h at this tick start). gaps.md unchanged since 2026-04-30T21:40:59Z (commit 61d666e). Reparse inventory unchanged from b0475. 5th substantive tick of UTC date 2026-05-03 (rolls forward from b0475 at 01:37Z).
...
All 89 ZMCC missing candidates remain deferred under v0.3.1-locked reason codes — none addressable by parser_v0.3.1. Empirically re-confirmed across b0375..b0475; verbatim per b0475 audit. ZMSC slice fully ingested across the 24 case HTML files (the additional HTML is the listing index page — not part of the ingestion-target inventory). Inventory exhausted; no fresh fetches initiated this tick.

### Deferred-reason histogram (gaps.md grep, unchanged from b0475)
| Reason code | Count |
|-------------|------:|
...
- 101 consecutive substantive audit-only ticks (b0375..b0383 + b0385..b0476; excludes the failed-pull tick between b0442 and b0443 and the aborted-pull tick at 19:33:50Z between b0464 and b0465).
- UTC date 2026-05-03 (5th substantive tick of new day; rolls forward from b0475 at 01:37Z).
...
## Recommendation (unchanged from b0441..b0475)
...
Expect another zero-length `.git/refs/remotes/origin/main.lock.bak.b0476_post.<ts>Z` ref to be created by host between this tick's push and the next tick's pull. Recovery is the same one-shot SHA-copy used in b0468/b0474/b0475/b0476. Recommend the host-side `find .git -name "*.lock*" -delete` extension still pending Peter sign-off.
