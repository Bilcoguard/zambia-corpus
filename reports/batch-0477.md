# Batch 0477 — Audit-only tick (102nd substantive consecutive)

- **Tick start (UTC):** 2026-05-03T03:04Z
...
- **Tick note:** initial cleanup glob `find .git -name "*.lock" -delete; find .git -name "*.lock.bak" -delete` ran cleanly with carry-forward `Operation not permitted` on host-locked `.git/objects/maintenance.lock` and on all `*.lock.bak.<timestamp>` accumulated under `.git/refs/remotes/origin/` (sandbox cannot unlink — same behaviour as b0460..b0476). First `git pull --ff-only` failed with `fatal: bad object refs/remotes/origin/main.lock.bak.b0476_retry1_20260503T023920170878833Z` — that ref file (and its sibling `.lock` file) were zero-length, plus an additional zero-length `test_z` ref that was created by an earlier write-permission probe in this same tick. Sandbox `unlink` is denied for files under `.git/refs/`, so the recovery path used in b0468/b0474/b0475/b0476 was repeated and broadened: copied current `.git/refs/remotes/origin/main` SHA (`d528625`) into every zero-length ref under `.git/refs/remotes/origin/` to convert each from a "bad object" into a duplicate-content ref (cosmetic clutter only — git tolerates duplicates). Re-pull then returned `Already up to date.` cleanly. approvals.yaml and BRIEF.md unchanged since 2026-04-30T15:36:40Z (commit b24a938; ~3d 11.5h at this tick start). gaps.md unchanged since 2026-04-30T21:40:59Z (commit 61d666e). Reparse inventory unchanged from b0476. 6th substantive tick of UTC date 2026-05-03 (rolls forward from b0476 at 02:37Z).
...
All 89 ZMCC missing candidates remain deferred under v0.3.1-locked reason codes — none addressable by parser_v0.3.1. Empirically re-confirmed across b0375..b0476; verbatim per b0476 audit. ZMSC slice fully ingested across the 24 case HTML files (the additional HTML is the listing index page — not part of the ingestion-target inventory). Inventory exhausted; no fresh fetches initiated this tick.

### Deferred-reason histogram (gaps.md grep, unchanged from b0476)
| Reason code | Count |
|-------------|------:|
...
- 102 consecutive substantive audit-only ticks (b0375..b0383 + b0385..b0477; excludes the failed-pull tick between b0442 and b0443 and the aborted-pull tick at 19:33:50Z between b0464 and b0465).
- UTC date 2026-05-03 (6th substantive tick of new day; rolls forward from b0476 at 02:37Z).
...
## Recommendation (unchanged from b0441..b0476)
...
Expect another zero-length `.git/refs/remotes/origin/main.lock.bak.b0477_post.<ts>Z` ref to be created by host between this tick's push and the next tick's pull. Recovery is the same one-shot SHA-copy used in b0468/b0474/b0475/b0476/b0477. Recommend the host-side `find .git -name "*.lock*" -delete` extension still pending Peter sign-off; this tick demonstrated the broadened sweep (heal every zero-length ref under `.git/refs/remotes/origin/` in one pass) is robust against the test-probe artefact (`test_z`) inadvertently created earlier in the tick.
