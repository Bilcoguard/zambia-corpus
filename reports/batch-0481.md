# Batch 0481 — Audit-only tick (106th substantive consecutive)

- **Tick start (UTC):** 2026-05-03T05:03Z
...
- **Tick note:** Initial cleanup glob `find .git -name "*.lock" -delete; find .git -name "*.lock.bak" -delete` ran cleanly with the by-now-routine carry-forward `Operation not permitted` on host-locked `.git/objects/maintenance.lock` (sandbox cannot unlink — same behaviour as b0460..b0480). `git pull --ff-only` returned `Already up to date.` on the first attempt this tick (no orphan-ref recovery needed; the b0479 broadened mv-into-`_orphan_refs_parked/` sweep continues to keep the refs tree clean through the b0480 push cycle, now confirmed across two consecutive clean pulls). approvals.yaml and BRIEF.md unchanged since 2026-04-30T15:36:40Z (commit b24a938; ~3d 13h at this tick start). gaps.md unchanged since 2026-04-30T21:40:59Z (commit 61d666e). Reparse inventory unchanged from b0480. 10th substantive tick of UTC date 2026-05-03 (rolls forward from b0480 at 04:36Z).
...
All 89 ZMCC missing candidates remain deferred under v0.3.1-locked reason codes — none addressable by parser_v0.3.1. Empirically re-confirmed across b0375..b0480; verbatim per b0480 audit. ZMSC slice fully ingested across the 24 case HTML files. Inventory exhausted; no fresh fetches initiated this tick.

### Deferred-reason histogram (gaps.md grep, unchanged from b0480)
| Reason code | Count |
|-------------|------:|
...
- 106 consecutive substantive audit-only ticks (b0375..b0383 + b0385..b0481; excludes the failed-pull tick between b0442 and b0443 and the aborted-pull tick at 19:33:50Z between b0464 and b0465).
- UTC date 2026-05-03 (10th substantive tick of new day; rolls forward from b0480 at 04:36Z).
...
## Recommendation (unchanged from b0441..b0480)
...
Two consecutive clean pulls (b0480, b0481) confirm the b0479 broadened mv-into-`_orphan_refs_parked/` sweep cleared the residual refs-tree contamination. If the next tick's pull fails with `fatal: bad object refs/heads/main.lock.bak.b0481_<ts>` (or remotes/origin equivalent), repeat the broadened mv-into-`_orphan_refs_parked/` sweep against both `refs/heads/` and `refs/remotes/origin/`. Host-side `rm -rf .git/_orphan_refs_parked/` plus `find .git -name "*.lock*" -delete` and `git gc --prune=now` extension still pending Peter sign-off.
