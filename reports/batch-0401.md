# Batch 0401 — Phase 5 Audit-Only Idle Tick

**Tick start (UTC):** 2026-05-01T11:04:03Z
**Tick id:** batch-0401
...
This is the **26th consecutive substantive audit-only tick** (b0375..b0383 plus b0385..b0401).
...
~19h 28m at tick start, per `git log -1 --format=%cI -- approvals.yaml`). No human approval
...
- 6/6 random-sample `source_hash` recompute matches against on-disk raw bytes (5 under
  `raw/zambialii/judgments/...`, 1 SCZ pilot under `raw/pilot/judiciary-zm/...` —
  expected layout).
- 0 unresolved `cited_authorities`, `amended_by`, or `repealed_by` in-corpus references.

## Fresh DESC sweep — still deferred

Fresh DESC sweep continues to be deferred per b0376–b0400 rationale:
...
## Escalation (26th consecutive substantive idle tick)
...
`_stale_locks_b03**_*.lock.bak` quarantine residue at the repo root: **6,384 files**
visible at tick start (count includes pre-b0386 dated and post-b0386 timestamped files;
the apparent jump vs. b0400's "77" reflects a more inclusive `ls _stale_locks_*` count
rather than new growth this tick). Harmless to git operations and worker logic; suggest a
single host-side `rm -f ~/KateWestonCorpus/corpus/_stale_locks_*` (Peter on Mac shell) at
a convenient maintenance window. Worker has not attempted in-sandbox cleanup because every
