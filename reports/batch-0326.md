...
**Wall-clock:** ~3 min
**Parser version:** 0.5.0 (probe-only — same as b0290..b0325)
...
b0290..b0326 (thirty-seventh consecutive tick).

## Tick context

Pre-tick lock cleanup ran cleanly. A leftover `.git/index.lock` was
present at tick start (carry-over from interrupted prior op); rotated
to `_stale_locks/index-<ts>.lock` mid-tick before the integrity probe
of `git status records/`. Benign `Operation not permitted` on
...
Page sha256 this tick: `ffe4cc04a68a9afb8b65031952536425b83af0239688f8f6359e94989311864b`.
...
| Source     | sha256 | Match |
|------------|--------|-------|
| zambialii  | `fce67b697ee4ef44e0694134e23108c2701b5e7661eed885013efb9b75dcd8f0` | yes (unchanged from b0193..b0325) |
| parliament | `278e83bcf567badfebcdea4d5d20ca9898e4449fe4eb2e3b5a08227b4ca9b762` | yes (unchanged from b0290..b0325) |
...
See `_work/batch_0326_probe.json`.

## Cost / budget

- Today (UTC) fetches before tick: 64 (b0310..b0325 ran earlier on 2026-04-28)
- Fetches this tick: 4 (2 robots + 2 page probes)
- Today (UTC) fetches after tick: 68 / 2000 (~3.4%)
...
Phase 4 appears at upstream steady state for the **thirty-seventh**
consecutive tick (b0290..b0326). Worker does **not** flip
