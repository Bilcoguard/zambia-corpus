...
**Parser version:** 0.5.0 (probe-only — same as b0290..b0339)
...
b0290..b0340 (fifty-first consecutive tick).
...
This tick the sandbox session is `sleepy-amazing-keller`; the probe
`ROOT` was rewritten accordingly. The probe ran in three stages
(`_work/batch_0340_stage_{a,b,c}.py` — zambialii fetches, parliament
fetches, parse + integrity) to fit individual bash-tool wall-clock
budgets in this sandbox; the merged JSON output at
`_work/batch_0340_probe.json` is byte-for-byte equivalent to a
single-script run.
...
(UNCHANGED vs b0329..b0339 — feed surface and ranking are byte-for-byte
identical for the twelfth consecutive tick).
...
Page bytes: 38,208 (unchanged across all 51 steady-state ticks).
Page sha256 this tick: `8b1aa5902bce89dac22a0a84ddfb2f85a4c817501858882164aca71dd7b37387`
...
| Source     | sha256 | Match |
|------------|--------|-------|
| zambialii  | `fce67b697ee4ef44e0694134e23108c2701b5e7661eed885013efb9b75dcd8f0` | yes (unchanged from b0193..b0339) |
| parliament | `278e83bcf567badfebcdea4d5d20ca9898e4449fe4eb2e3b5a08227b4ca9b762` | yes (unchanged from b0290..b0339) |
...
See `_work/batch_0340_probe.json`.

## Cost / budget

- Today (UTC) fetches before tick: 40 (b0330..b0339)
- Fetches this tick: 4 (2 robots + 2 page probes)
- Today (UTC) fetches after tick: 44 / 2000 (~2.2%)
...
Fifty-first consecutive steady-state tick. Phase 4 cannot meaningfully
