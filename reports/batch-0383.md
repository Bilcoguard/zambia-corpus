...
- `git pull --ff-only` reports "Already up to date". (`.git/objects/maintenance.lock` warned-but-skipped — non-blocking carry-forward; `git` exits 0.)
...
- UTC date: 2026-05-01 (3rd tick of new UTC day; b0380, b0381, b0382 were 1st/2nd; b0383 is 3rd). Today's cumulative fetches at tick start = 0 / 2000.

## Inventory snapshot (programmatic, identical to b0378–b0382)
...
Identical inventory to b0378 / b0379 / b0380 / b0381 / b0382 — no drift on tick 3 of the new UTC day.

## Programmatic gaps.md cross-check

- Iterated every raw file under `raw/zambialii/judgments/zmcc/{2021..2026}/`.
- Extracted canonical `(year, num)` from each filename via `judgment-zm-(\d{4})-zmcc-(\d+)-`.
- Compared against the same key set extracted from `records/judgments/zmcc/` JSON records.
- 89 missing candidates found; **89/89 referenced in `gaps.md`** under v0.3.1-specific reason codes (zero uncatalogued candidates). Identical to b0378–b0382 — no drift.

## Why the same three options are still on the table this tick

Inputs unchanged from b0379–b0382. Re-stating the unblock list verbatim so the next human review has the full context inline:
...
Same two considerations as b0379–b0382, unchanged because no inputs have changed:
...
- Files committed this tick: `costs.log`, `worker.log`, `reports/batch-0383.md`.
