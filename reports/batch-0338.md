...
**Parser version:** 0.5.0 (probe-only — same as b0290..b0337)
...
b0290..b0338 (forty-ninth consecutive tick).
...
This tick the probe script was rebuilt under a fresh sandbox session
(session mount rewritten to `/sessions/zealous-practical-pasteur/...`).
The in-corpus check was hardened to walk the flat top-level of
`records/acts/` explicitly in addition to the per-year subdirectory:
the b0337 cross-check note documented this two-layout behaviour but
the inline implementation in b0290..b0337 only inspected
`records/acts/<yr>/` and would have miscounted novelty if the
top-level layout were re-introduced. Behaviour is now aligned with
the documented contract — and the probe-result count (15/15 zam,
20/20 parl in corpus, novel = 0) matches the b0337 baseline.
...
Page bytes: 95,392 (unchanged vs b0329..b0337).
Page sha256: `507f7542eddc74c7089f669398fc86861aea75d60dc9dabe0fbddb922d79321c`
(UNCHANGED vs b0329..b0337 — feed surface and ranking are byte-for-byte
identical for the tenth consecutive tick).
...
Page bytes: 38,208 (unchanged across all 49 steady-state ticks).
Page sha256 this tick: `76246fb01b3dc6abe3c655e9c4593e0980b631fcb697cb6f742c0ecefef24edb`
...
| Source     | sha256 | Match |
|------------|--------|-------|
| zambialii  | `fce67b697ee4ef44e0694134e23108c2701b5e7661eed885013efb9b75dcd8f0` | yes (unchanged from b0193..b0337) |
| parliament | `278e83bcf567badfebcdea4d5d20ca9898e4449fe4eb2e3b5a08227b4ca9b762` | yes (unchanged from b0290..b0337) |
...
See `_work/batch_0338_probe.json`.

## Cost / budget

- Today (UTC) fetches before tick: 32 (b0330..b0337)
- Fetches this tick: 4 (2 robots + 2 page probes)
- Today (UTC) fetches after tick: 36 / 2000 (~1.8%)
...
Forty-ninth consecutive steady-state tick. Phase 4 cannot meaningfully
