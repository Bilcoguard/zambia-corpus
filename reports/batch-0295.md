...
**Wall-clock:** ~4 min
**Parser version:** 0.5.0 (probe-only — same as b0290..b0294)
...
## Tick context — recovery

Starter pre-tick lock cleanup (`find -name "*.lock" -delete`,
`find -name "*.lock.bak" -delete`) ran cleanly this tick — no leftover
b0294 stale ref or sandbox unlink blocker. `git pull --ff-only`
returned "Already up to date" on first attempt.

Note: a benign `unable to unlink ...maintenance.lock: Operation not
permitted` warning was emitted by `git pull` (sandbox cannot delete
files in `.git/objects/`) but the pull itself succeeded cleanly —
this is not a recovery condition.

The b0294 close note recommended a one-time host-side
`rm -f .git/refs/remotes/origin/main.lock*` to permanently clear the
b0289..b0294 stale orphan refs. **This tick suggests the host operator
has performed it (or something equivalent) — no orphan ref blocking
this tick.**
...
Cross-check vs `records/acts/`: **all 13 already in corpus** (matched
...
| Source     | sha256 | Match |
|------------|--------|-------|
| zambialii  | `fce67b697ee4ef44e0694134e23108c2701b5e7661eed885013efb9b75dcd8f0` | yes (unchanged from b0193..b0294) |
| parliament | `278e83bcf567badfebcdea4d5d20ca9898e4449fe4eb2e3b5a08227b4ca9b762` | yes (unchanged from b0290..b0294) |
...
After b0290..b0295, **every priority_order sub-phase is exhaustively
...
Until one of the above is actioned, future ticks (b0296+) will continue
to idle on phase_4_bulk with re-run probes that only re-confirm steady
state.

## Integrity

`_work/batch_0295_integrity.py` — **PASS**:
...
- Today (2026-04-27 UTC) fetches at tick start: 68 (per costs.log)
- Today (2026-04-27 UTC) fetches at tick end:   72/2,000 (~3.6%)
  - This tick: 4 fetches.
...
- `scripts/batch_0295.py` (PICKS=[] documentation skeleton)
- `_work/batch_0295_*` (discover, recent HTML, parliament HTML,
  robots caches, integrity script, probe.json)
- `reports/batch-0295.md` (this file)
