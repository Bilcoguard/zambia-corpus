#!/usr/bin/env python3
"""
Batch 0183 - Phase 4 sis_tax.

Eight tax-related SIs from the /legislation?alphabet=I novel queue
(see _work/batch_0182_candidates.json).

Priority from batch 0182 next-tick plan:
  - si/2017/074  Income Tax (Exemption of tax on interest payments to
                 overseas lenders by Maamba Collieries Limited) Order
  - si/2020/011  Income Tax (Royal Haskoning DHV (PTY) Limited) Order
  - si/2022/032  Income Tax (John Snow Health Zambia Limited) Order
  - si/2024/065  Income Tax (GOPA Infra GmbH) Order, 2024
  - si/2025/089  Income Tax (GOPA Infra GmbH) Order, 2025

Plus three more modern tax SIs from the same novel queue:
  - si/2017/043  Income Tax (Batoka Hydro-Electric / Kariba Dam) Regulations
  - si/2019/025  Income Tax (Suspension on Treasury Bill / Bond interest)
  - si/2025/092  Income Tax (Konoike Construction Company Ltd) Order

Ingest fetches: 8 * 2 = 16.
"""
import hashlib
import io
import json
...
WORKSPACE = "/sessions/adoring-admiring-fermat/mnt/corpus"
os.chdir(WORKSPACE)
UTC = timezone.utc
...
def main():
    ...
    if os.path.exists(prev_path):
        try:
            ...
            pass
    # Internal soft deadline: 40s per python invocation (to fit 45s bash timeout)
