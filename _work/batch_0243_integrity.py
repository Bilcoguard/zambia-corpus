"""Integrity checks for batch 0243.
...
# Load all batch 0243 ok records
ok_records = []
for i in range(8):
    fp = f'_work/batch_0243_one_{i}.json'
    if not os.path.exists(fp):
        print(f'MISSING {fp}')
        sys.exit(2)
    d = json.load(open(fp))
    if d.get('status') != 'ok':
        print(f'NOT-OK {fp}: {d.get("status")}')
        sys.exit(2)
...
with open('_work/batch_0243_integrity.json','w') as f:
    _j.dump(out, f, indent=2)
print("wrote _work/batch_0243_integrity.json")
