...
CERT_OUT = "_work/trust_bundle_0312.pem"
...
for name, url, delay in PROBES:
    ...
    if name == "zambialii":
        ...
        Path("_work/zambia_recent_b0312.html").write_bytes(body)
    else:
        parl_bytes = len(body)
        ...
        Path("_work/parliament_acts_b0312.html").write_bytes(body)
    time.sleep(delay)
...
Path("_work/batch_0312_probe.json").write_text(json.dumps(results, indent=2))
print(json.dumps({
