"""Batch 0301 — minimal upstream-refresh probe (12th consecutive steady-state tick).
...
WORKSPACE = "/sessions/optimistic-charming-keller/mnt/corpus"
...
CA_BUNDLE = os.path.join(WORKSPACE, "_work/_ca_bundle_b0301.pem")
...
for u, exp in EXPECTED.items():
    ...
    match = (h == exp)
    robots_results[u] = {"sha256": h, "match": match, "len": len(r.content)}
    print(u, "len=" + str(len(r.content)), "sha256=" + h, "match=" + str(match))
...
with open(os.path.join(WORKSPACE, "_work/zambia_recent_b0301.html"), "w") as f:
    f.write(zambia_html)
with open(os.path.join(WORKSPACE, "_work/parliament_acts_b0301.html"), "w") as f:
    f.write(parliament_html)
...
state = {
    ...
    "robots_match_all": all(v["match"] for v in robots_results.values()),
}
with open(os.path.join(WORKSPACE, "_work/batch_0301_probe.json"), "w") as f:
    json.dump(state, f, indent=2)

with open(os.path.join(WORKSPACE, "costs.log"), "a") as f:
    for line in cost_lines:
        f.write(line + "\n")

print("\n--- DONE ---")
print(f"cost_lines written: {len(cost_lines)}")
print(f"records existing count (acts): {len(existing)}")
