"""Ingest a single PICK index from batch_0251 - per-record subprocess pattern.
Mirror of batch_0250 ingest_one: fetch HTML, extract title from <h1>/og:title,
slugify, fetch + parse PDF, write JSON record + raw HTML/PDF.

Invoke via:
    python3 _work/batch_0251_ingest_one.py <idx> [sleep_first=1]
Writes _work/batch_0251_one_<idx>.json with status=ok|skip|fail.
"""
...
from batch_0251 import PICKS, slugify, fetch, CRAWL, PARSER_VERSION, UA, HEADERS
...
def fetch_one(idx, sleep_first=False):
    ...
    tmp_html = f'{raw_dir}/_tmp-batch-0251-{yr}-{num}.html'
...
if __name__ == '__main__':
    ...
    out = f'_work/batch_0251_one_{idx}.json'
