"""Batch 0299 integrity check (probe-only refresh tick).
...
The relevant "soft" integrity checks for a probe-only tick are:
  ...
  C1: records/sis/ JSON count unchanged (539).
  C2: records/acts/ JSON count unchanged (1169).
...
def main():
    ...
    # Load probe data
    probe = json.loads((ROOT / "_work" / "batch_0299_probe.json").read_text())
    ...
    print("\nbatch 0299 integrity: PASS (probe-only; CHECK1..CHECK6 N/A — zero record writes)")
