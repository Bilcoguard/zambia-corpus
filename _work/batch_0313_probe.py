"""Batch 0313 — Phase 4 / minimal upstream-refresh tick (24th consecutive steady-state tick).
...
CERT_OUT = "_work/trust_bundle_0313.pem"
...
for name, url, delay in PROBES:
    ...
    if name == "zambialii":
        ...
        Path("_work/zambia_recent_b0313.html").write_bytes(body)
    else:
        parl_bytes = len(body)
        ...
        Path("_work/parliament_acts_b0313.html").write_bytes(body)
    time.sleep(delay)
...
Path("_work/batch_0313_probe.json").write_text(json.dumps(results, indent=2))
print(json.dumps({
