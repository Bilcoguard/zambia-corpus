"""Ingest a single PICK index from batch_0250 - per-record subprocess pattern.
Mirror of batch_0248 but with title extracted from HTML (og:title / <h1>).
Invoke via:
    python3 _work/batch_0250_ingest_one.py <idx> [sleep_first=1]
Writes _work/batch_0250_one_<idx>.json with status=ok|skip|fail.
"""
...
from batch_0250 import PICKS, slugify, fetch, CRAWL, PARSER_VERSION, UA, HEADERS

MAX_PDF_BYTES = 4_500_000


def extract_title(soup, default=""):
    """Try og:title, <title>, <h1>, in that order."""
    ...
    if soup.title and soup.title.string:
        ...
        # Strip site suffix like " | Zambia Legal Information Institute"
...
def extract_parent_act(soup, fallback=""):
    """Try to read parent act from breadcrumb / og:title / structure."""
    # Look for breadcrumb-like links
    for a in soup.find_all('a', href=True):
        href = a['href']
        # zambialii Acts have /akn/zm/act/<year>/<num> not /si/
...
def fetch_one(idx, sleep_first=False):
    ...
    # ---- HTML fetch (or reuse) ----
    # Use a temporary record_id (without title) to seed the path; will move after
    # we know the title.
    tmp_html = f'{raw_dir}/_tmp-batch-0249-{yr}-{num}.html'
    ...
    # Move tmp_html to final path
    html_path = f'{raw_dir}/{record_id}.html'
    if not os.path.exists(html_path):
        os.rename(tmp_html, html_path)
    else:
        # final path already exists; remove temp
        try:
            os.remove(tmp_html)
        except OSError:
            pass

    # Update fetches_done with final path
    ...
    if not pdf_url:
        # Fall back to og:url canonical effective date
        date_match = None
        for meta in soup.find_all('meta'):
...
if __name__ == '__main__':
    ...
    out = f'_work/batch_0250_one_{idx}.json'
