"""Ingest a single index from batch_0225 PICKS, with strict timing.

Designed for 45s sandbox call cap: per-call fetches one HTML + one PDF + parses.
Skips PDFs > 4MB (likely scanned image PDFs). Writes a single result file.
"""
...
from batch_0225 import PICKS, slugify, fetch, CRAWL, PARSER_VERSION, UA, HEADERS

MAX_PDF_BYTES = 4_500_000  # skip larger PDFs (likely scanned/image)
...
def fetch_one(idx, sleep_first=False):
    ...
    # If raw HTML and PDF already exist on disk (from a partial prior run), reuse
...
if __name__ == '__main__':
    ...
    out = f'_work/batch_0225_one_{idx}.json'
