#!/usr/bin/env python3
"""
Batch 0142 — Phase 4 bulk ingestion.

Continuation of the N-P-R block work. Targets sourced from the leftover
`_work/batch_0140_discovery.json` candidates that were not ingested in
batches 0140/0141, plus additional primary statutes resolved via the
ZambiaLII search API for common N-P block queries ("Employment",
"Workers' Compensation", "Pharmacy", "Nurses"). Eight targets selected,
all primary statutes (no amendment acts, no appropriation acts, no
repeal-only acts), all verified as NOT yet in HEAD by (year, num) tuple
against `git ls-tree -r HEAD records/acts/`:

  1. 2002/11  — Road Traffic Act, 2002 (repealer of 1958/37)
  2. 1966/20  — Local Courts Act, 1966
  3. 1957/8   — Lotteries Act, 1957
  4. 1991/2   — Electoral Act, 1991 (predecessor of 2006/12)
  5. 1977/22  — Medical and Allied Professions Act, 1977
  6. 1999/10  — Workers' Compensation Act, 1999
  7. 1965/57  — Employment Act, 1965
  8. 2004/14  — Pharmaceutical Act, 2004

All Acts fetched via AKN URL pattern
...
TARGETS = [
    (2002, 11, "road-traffic-act-2002"),
    (1966, 20, "local-courts-act-1966"),
    (1957, 8,  "lotteries-act-1957"),
    (1991, 2,  "electoral-act-1991"),
    (1977, 22, "medical-and-allied-professions-act-1977"),
    (1999, 10, "workers-compensation-act-1999"),
    (1965, 57, "employment-act-1965"),
    (2004, 14, "pharmaceutical-act-2004"),
]
...
def write_batch_report(records, gaps, total_sections, fetches, integrity_passed):
    ...
    lines = [
        ...
        "## Strategy",
        "",
        "Continuation of the ZambiaLII AKN-URL ingestion from batches "
        "0134–0141. Targets sourced from the leftover "
        "`_work/batch_0140_discovery.json` candidates (Road Traffic 2002/11, "
        "Local Courts 1966/20, Lotteries 1957/8, Electoral 1991/2) plus "
        "additional primary statutes resolved via the ZambiaLII search API "
        "(`/search/api/documents/?search=<q>&nature=Act`) for the N-P block "
        "(Medical and Allied Professions 1977/22, Workers' Compensation "
        "1999/10, Employment 1965/57, Pharmaceutical 2004/14). Eight "
        "targets, all primary statutes (no amendment acts, no appropriation "
        "acts, no repeal-only acts), all verified as NOT in HEAD by "
        "(year, num) tuple against `git ls-tree -r HEAD records/acts/` "
        "before selection. Fetched via AKN URL pattern with PDF fallback "
        "when HTML returned fewer than 2 parsed sections.",
        "",
        "## Committed records",
        "",
    ...
    lines.extend([
        "## Notes",
        "",
        "- Road Traffic Act 2002/11 was flagged in batch 0141 as the "
        "repealer of the Roads and Road Traffic Act 1958/37 (already in "
        "HEAD). Both now in corpus — repeal-chain cross-ref links deferred "
        "to a future amendment-linking batch.",
        "- Electoral Act 1991/2 is the direct predecessor of the Electoral "
        "Act 2006/12 (already in HEAD from batch 0141). Historical primary "
        "included for completeness — repeal-chain cross-refs deferred.",
        "- Local Courts Act 1966/20 remains the governing statute for the "
        "subordinate local courts system; no repealer identified.",
        "- Lotteries Act 1957/8 is pre-independence (colonial-era) "
        "legislation but remains in the Laws of Zambia (Cap 411 range) "
        "per ZambiaLII metadata.",
        "- Medical and Allied Professions Act 1977/22 was largely "
        "superseded by the Health Professions Act 2009/24 (already in "
        "HEAD) and the Medical Laboratory Profession Act 2017; "
        "repeal-chain cross-refs deferred.",
        "- Employment Act 1965/57 is the colonial/early-independence "
        "foundation of Zambian labour law; superseded in substantial part "
        "by the Employment Code Act 2019/3 (in HEAD). Historical primary "
        "preserved for section-level citation in legacy case law.",
        "- Next tick: continue N-P-R block — National Assembly (Powers "
        "and Privileges) Cap 12 range, National Payment Systems 2007, "
        "Pensions and Insurance Authority Act predecessors, Postal Services "
        "Act 2009, Probate and Administration of Estates Act, Registered "
        "Designs Act, Rent Act. Discovery probes via ZambiaLII search API "
        "remain the preferred resolution path.",
        "",
    ])
