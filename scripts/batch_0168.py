#!/usr/bin/env python3
"""
Batch 0168 — Phase 4 bulk ingestion.

Probe-only pass. Per batch 0167 next-tick plan (probe yield = 3
new primary parents — above the 2-parent alphabetical-fallback
threshold, so continue primary-statute sweep with fresh narrower
rotation). SCOPE REDUCED: the sandbox per-call bash timeout is
45 s, and a full 8-probe + 8-ingest run plus 5-sec rate limits
cannot complete inside that window. This tick runs a
2-probe / MAX_RECORDS=2 sub-batch with PDF fallback DISABLED
(HTML-only — sparse-HTML candidates are gapped, not PDF-fetched):

    Probe rotation (this tick): national assembly,
    parliament, cooperatives, electoral, food safety.
    Previously run in this batch's earlier sub-tick attempt
    (0-yield, logged): agriculture, public health.
    Remaining rotation query deferred: elections.
...
Mirrors batch 0166 processor logic (HTML/AKN + pre-write
B-POL-ACT-1 title filter + OCR variants) but with PDF fallback
DISABLED for this tick. Wall-clock guarded to stay inside the
sandbox per-call 45-second bash timeout (~30s budget for Stage 2
with 15s headroom for parse, integrity checks, and exit).
...
WALL_CLOCK_BUDGET_SECS = 30  # 30 seconds — sandbox per-call bash timeout is 45s

# No seed candidates deferred into this batch from 0167.
SEED_CANDIDATES = []
...
def process_act(year, number, slug_hint):
    ...
    if len(sections) < 2:
        return None, None, (
            "HTML sparse (sections<2) and PDF fallback disabled "
            "in this tick to respect the sandbox 45s bash timeout"
        )
...
def write_batch_report(records, gaps, total_sections, fetches, integrity_passed,
    ...
    lines = [
        ...
        "with FIVE fresh narrower rotation queries per the "
        "batch-0167 next-tick plan: national assembly, "
        "parliament, cooperatives, electoral, food safety "
        "(REDUCED from the full 8-query rotation to fit the "
        "sandbox 45s per-call bash timeout; `agriculture` and "
        "`public health` were run in an earlier sub-tick "
        "attempt with 0 yield — both probes returned only "
        "amendment-act variants, logged). PDF fallback is "
        "DISABLED — sparse-HTML candidates are gapped, not "
        "PDF-fetched. Hits surviving "
        ...
        "fallback is DISABLED in this tick — sparse-HTML "
        "candidates are gapped (not PDF-fetched) to keep the "
        "run inside the sandbox per-call 45s bash timeout. "
        "No SEED candidates this batch; "
        "no unconditional repeal-chain links are pre-declared.",
        "",
        "## Committed records",
    ...
    lines.extend([
        ...
        "continue the primary-statute sweep and pick up the "
        "remaining queries deferred from this tick: "
        "cooperatives, electoral, elections, national "
        "assembly, parliament, food safety.",
        "",
    ])
