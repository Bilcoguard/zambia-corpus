#!/usr/bin/env python3
"""
Batch 0151 — Phase 4 bulk ingestion.

Continuation of the batch 0150 next-tick plan. Probes the ZambiaLII
search API for primary-parent Acts in the Wills / Succession /
Intestate, Insurance / Pensions, and Co-operatives families that
are not yet in HEAD. Mirrors batch 0150 processor logic
(probe-and-ingest + pre-write B-POL-ACT-1 title filter + PDF
fallback when HTML sparse).

Also applies one repeal-chain link verified against HEAD:
  Trade Marks Act 1957/14 -> Trade Marks Act 2023/11
Rationale: the 2023 Trade Marks Act record in HEAD repeatedly
references "the repealed Act Cap.401" in its transitional
provisions; the 1957 Act is the Cap.401 parent. Both records are
already in HEAD, so this is a pure link-fill, not a new ingestion.
"""
...
REPEAL_LINKS = [
    (
        "records/acts/1957/act-zm-1957-014-trade-marks-act-1957.json",
        "act-zm-2023-011-the-trade-marks-act-2023",
        "Trade Marks Act 1957 (Cap. 401) superseded by Trade Marks Act 2023 (No. 11 of 2023); 2023 Act transitional provisions reference 'the repealed Act Cap.401'.",
    ),
]
...
def write_batch_report(records, gaps, total_sections, fetches, integrity_passed,
    ...
    lines = [
        ...
        "## Strategy",
        "",
        "Continuation of the batch 0150 next-tick plan — primary-statute "
        "sweep for Wills / Succession / Intestate, Insurance / Pensions, "
        "and Co-operative / Friendly Societies parents. Candidate (year, "
        "number) pairs were surfaced by probing the ZambiaLII search API "
        "(`/search/api/documents/?search=<q>&nature=Act`) for "
        f"{len(PROBE_QUERIES)} hint queries at batch-{BATCH_NUM}-probe; each "
        "hit was cross-referenced against HEAD by (year, number) tuple via "
        "`git ls-tree -r HEAD records/acts/`. Slots whose AKN-page title "
        "contained `amendment`, `appropriation`, `repeal`, `supplementary`, "
        "`validation`, or `transitional` were rejected pre-write without "
        "producing any raw or record file. The processor fetches each "
        "surviving candidate's AKN page in priority order and stops at "
        f"MAX_RECORDS={MAX_RECORDS} accepted primary statutes. PDF fallback "
        "is invoked only when the HTML returns fewer than 2 parsed sections.",
        "",
        "## Committed records",
        "",
    ...
    lines.extend([
        "## Notes",
        "",
        "- B-POL-ACT-1 title filter applied pre-write: any candidate whose "
        "AKN-page title contained `amendment`, `appropriation`, `repeal`, "
        "`supplementary`, `validation`, or `transitional` was rejected "
        "without producing a raw or record file.",
        "- Next tick: continue the primary-statute sweep with further probe "
        "queries on Insurance, Copyright, Patents, Companies (secondary "
        "caps), and Land / Lands Tribunal where gaps remain; also revisit "
        "the Patents Act Cap.400 parent and the Copyright and Performance "
        "Rights Act Cap.406 parent.",
        "",
    ])
