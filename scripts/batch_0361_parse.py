#!/usr/bin/env python3
"""Batch 0361 parser-only step (REPARSE-FIRST PASS).

Re-parses 8 ZMCC raw HTML+PDF pairs already on disk that were deferred
in earlier batches (b0344..b0359) under parser_version 0.3.0's
"outcome_not_inferable_under_tightened_policy" reason. Frozen baseline
from scripts/batch_0360_parse.py (parser_v0.3.1, pdf-tail-2pages
fallback). Per the locked-in reparse_first policy in approvals.yaml
(2026-04-30), this batch consumes ZERO fresh fetch budget — all raw
bytes are already on disk under raw/zambialii/judgments/zmcc/.

Slice: top of the deferred queue, year-DESC then num-DESC, picking
candidates whose record was never written:
  zmcc 2026/1, 2025/33, 2025/32, 2025/30, 2025/28, 2025/25,
  2025/24, 2025/23.
...
TARGETS = [
    ...
    # before locking the slice — see worker.log for b0361).
    # Batch 0361 (REPARSE PASS): top of deferred queue, year-DESC
    # then num-DESC. All raw HTML+PDF already on disk from earlier
    # batches; this run consumes zero fetch budget.
