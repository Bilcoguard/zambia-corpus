...
only BATCH, WORKSPACE, and seed (to embed BATCH) constants changed
for this tick.
...
- Sample is deterministic: seed embeds <UTC date>+<BATCH>, so the
  same date+batch always produces the same sample (re-runnable for audit),
  and successive batches on the same day pick different samples.
...
- For each fetched URL, computes sha256 and emits one of three statuses:
    ...
    fetch_error        — non-200 or transport error; no hash compared.
- Writes nothing to records/. Only mutation is appending to logs and
  emitting a JSON summary.
...
WORKSPACE = "/sessions/busy-vigilant-fermat/mnt/corpus"
