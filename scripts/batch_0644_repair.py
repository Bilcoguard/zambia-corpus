#!/usr/bin/env python3
"""b0644 — Repair worker batch.

Continues the b0643 (and b041–b042) pattern: body-only updates to
...
def quality_gate(body):
    ...
    markers = ["Act", "Regulations", "section", "Order", "ENACTED",
               "Statutory Instrument", "By-Laws", "Rules"]
...
def main():
    ...
    # FUSE bindfs blocks rm of corpus.sqlite-journal — without this, every
    # rollback-journal commit fails with "disk I/O error". Switch to MEMORY
    # journal so SQLite never tries to rm a journal file (b0644 fix).
