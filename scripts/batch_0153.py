#!/usr/bin/env python3
"""
Batch 0153 — Phase 4 bulk ingestion.

Continuation of the batch 0152 next-tick plan. Two-stage candidate
generation:

1. SEED (known year/number) — the 5 candidates that batch 0151 and 0152
   deferred by the MAX_RECORDS=8 cap: Carriage by Air Act 1968/59,
   Rhodesia Railways Loans Guarantee Act 1950/18, General Loans
   (Guarantee) Act 1964/51, National Savings and Credit Act 1972/24,
   Corrupt Practice Act 1980/14. These were surfaced and title-filtered
   in prior batch probes; their AKN slot identifiers are documented in
   gaps.md so no re-probe is needed.

2. PROBE (ZambiaLII search API) — Sale of Goods, Bills of Exchange,
   Juveniles, Hire Purchase, Stamp Duty, Insurance Cap. 392 parent
   (likely 1997). Any hits surviving HEAD + title filters fill the
   remaining batch slots (MAX_RECORDS=8).

Mirrors batch 0152 processor logic (HTML/AKN with PDF fallback +
pre-write B-POL-ACT-1 title filter). No repeal-chain links applied
this batch — deferred until Insurance Cap. 392 parent is confirmed.
"""
...
# Deferred candidates from batches 0151-0152 (batch cap reached).
# Each tuple: (year, number, slug_hint, source_note)
SEED_CANDIDATES = [
    (1968, 59, "carriage-by-air", "deferred batch 0151/0152"),
    (1950, 18, "rhodesia-railways-loans-guarantee", "deferred batch 0152"),
    (1964, 51, "general-loans-guarantee", "deferred batch 0152"),
    (1972, 24, "national-savings-and-credit", "deferred batch 0152"),
    (1980, 14, "corrupt-practice", "deferred batch 0152"),
]
...
def write_batch_report(records, gaps, total_sections, fetches, integrity_passed,
    ...
    lines = [
        ...
        "MAX_RECORDS-deferred candidates that batch 0151 and 0152 "
        "flagged in `gaps.md` (year/number already verified in prior "
        "probes): Carriage by Air 1968/59, Rhodesia Railways Loans "
        "Guarantee 1950/18, General Loans (Guarantee) 1964/51, "
        "National Savings and Credit 1972/24, Corrupt Practice 1980/14. "
        "Stage 2 probes the ZambiaLII search API for Sale of Goods, "
        "Bills of Exchange, Juveniles Act, Hire Purchase, Stamp Duty, "
        "and Insurance Act parent (Cap. 392, likely 1997); hits "
    ...
    if repeal_changes:
        ...
        lines.append(
            ...
            "until the Insurance Act Cap. 392 parent is confirmed."
        )
    lines.extend([
        "",
        "## Seed summary",
    ...
    lines.extend([
        ...
        "- Next tick: depending on seed outcomes, continue the "
        "primary-statute sweep — Juveniles Act parent (Cap. 53), "
        "Hire Purchase Act parent, Stamp Duty parent, Sale of Goods "
        "parent, Bills of Exchange parent, Insurance Cap. 392 parent "
        "(if not surfaced by this probe); also Patents Cap. 400 and "
        "Copyright Cap. 406 parents remain unresolved by batches "
        "0146 and 0152 probes.",
        "",
    ])
