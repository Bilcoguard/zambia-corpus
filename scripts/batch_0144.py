#!/usr/bin/env python3
"""
Batch 0144 — Phase 4 bulk ingestion.

Fresh discovery via ZambiaLII search API (results_html parse) for the
next-tick hint list from batch 0143: Public Order Act parent,
Matrimonial Causes Act pre-2007 originals, Postal Services Act 2009
(and prior), Probate and Administration of Estates, Registered Designs
parent(s), and pre-Employment-Code Industrial/Labour Relations
originals. Screening queries (executed in the batch 0144 probe step
and logged to costs.log / provenance.log):
...
For each query the `results_html` fragment was parsed for AKN
`/akn/zm/act/YYYY/NN` URLs, cross-referenced against HEAD by (year,
num) tuple via `git ls-tree -r HEAD records/acts/`, and up to 10
non-HEAD candidates selected. This script processes 10 candidates;
the processor rejects any whose canonical AKN-page title contains
amendment / appropriation / repeal / validation tokens (B-POL-ACT-1)
and stops at MAX_RECORDS=8 accepted primary statutes.

Each accepted target is fetched via the AKN URL pattern with a PDF
fallback when the HTML returns fewer than 2 parsed sections. All
probe and fetch events are logged to costs.log / provenance.log via
fetch_one.fetch().
"""
...
# (year, act_number, slug_hint_for_diagnostics). Ordered by strategic
# priority: explicit next-tick hints first, then parent-of-amendment
# plays, then older colonial parents.
TARGETS = [
    (2009, 22, "postal-services-act-2009"),
    (1955, 38, "public-order-act-1955"),
    (1987, 25, "registered-designs-1987"),
    (1983, 13, "industrial-relations-act-1983"),
    (1960, 39, "matrimonial-causes-1960"),
    (1994, 24, "postal-services-1994"),
    (1951,  2, "probate-administration-1951"),
    (1958, 12, "registered-designs-1958"),
    (2008,  8, "candidate-2008-008"),
    (1989,  6, "candidate-1989-006"),
]
...
def process_act(year, number, slug_hint):
    ...
    If the canonical AKN-page title contains an amendment /
    appropriation / repeal / validation token, the target is rejected
    as a primary-statute candidate (B-POL-ACT-1 filter) BEFORE any
    PDF fallback is attempted. The HTML fetch still counts against
    costs.log (we fetched it once), but no record or raw artefact is
    written and the call returns (None, None, reject_reason).
    """
    ...
    # Canonical title cleanup BEFORE reject check.
    if not title:
    ...
    if len(sections) < 2:
        ...
        if pr["status"] == 200 and pr["body_len"] > 500:
            ...
            if psections:
                sections = psections
                if ptitle and not title_rejected(ptitle):
                    # Prefer PDF title only if non-empty and not a reject
                    # token; HTML title already passed the filter.
                    pass
...
def write_batch_report(records, gaps, total_sections, fetches, integrity_passed):
    ...
    lines = [
        ...
        "## Strategy",
        "",
        "Continuation of the batch 0143 next-tick plan: resolve the "
        "Public Order parent, Matrimonial Causes pre-2007 originals, "
        "Postal Services Act 2009 (and its 1994 predecessor), Probate "
        "and Administration of Estates pre-1989 parent, Registered "
        "Designs parent(s), and pre-Employment-Code Industrial/Labour "
        "Relations originals. Candidate (year, num) pairs were "
        "obtained by parsing the `results_html` fragment of the "
        "ZambiaLII search API "
        "(`/search/api/documents/?search=<q>&nature=Act`) for the six "
        "hint queries; each hit was cross-referenced against HEAD by "
        "(year, num) tuple via "
        "`git ls-tree -r HEAD records/acts/`. Ten non-HEAD candidates "
        "were queued; the processor fetches each AKN page, applies a "
        "title-token filter (rejects `amendment`, `appropriation`, "
        "`repeal`, `supplementary`, `validation`) BEFORE any raw or "
        "record file is written, and stops at MAX_RECORDS=8 accepted "
        "primary statutes. PDF fallback is invoked only when the HTML "
    ...
    lines.extend([
        ...
        "PASS — no cross-refs in this batch",
    ...
    lines.extend([
        ...
        "without producing a raw or record file. The filter runs on "
        "the cleaned HTML title (whitespace-normalised, leading "
        "'ACT' prefix stripped).",
        "- Next tick: continue the primary-statute sweep — candidates "
        "still outstanding include the Electoral Commission parent "
        "Act, the National Pension Scheme Authority base Act, and the "
        "Road Traffic Act parent; plus any remaining pre-independence "
        "Cap-base statutes on Marriage, Chiefs, and Probate that "
        "survived this batch's title filter.",
        "",
    ])
