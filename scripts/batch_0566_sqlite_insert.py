#!/usr/bin/env python3
"""Batch 0566 — insert this tick's 1 ZMCC 2018 record into corpus.sqlite.

Updates `records`, `judgments_meta`, AND `records_fts`. Uses TMPDIR-routed
atomic copy (b0531 pattern) and PRAGMA journal_mode=TRUNCATE (b0557
workaround for virtiofs unlink restriction).
...
Note: orphan judgment-zm-2019-zmcc-24-mwiya-mutapwe-v-shomeno-dominic
is ALREADY in DB (inserted by abandoned b0565 attempt) and is therefore
not in NEW_IDS here. The orphan record JSON + raw bytes will be swept
into the b0566 git commit.
"""
import json
import pathlib
import shutil
...
NEW_IDS = [
    "judgment-zm-2018-zmcc-01-chilombo-v-hamaleke",
]
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="b0566_sqlite_") as tmp:
