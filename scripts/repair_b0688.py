#!/usr/bin/env python3
"""b0688 repair worker — ZambiaLII SI repair (continuation of b0685 drainage).

Continues draining the 89 acts/SIs with no body (all zambialii AKN-SI URLs)
remaining after b0685. Stages on /tmp to avoid FUSE EPERM on the 91%-full
workspace volume. Establishes the parser pattern from repair_b0685.py
(parser version repair-0.6.85) — no logic changes, just a fresh tick wrapper.

Parser version: repair-0.6.88.
"""
import sqlite3, urllib.request, urllib.error, ssl, re, os, sys, hashlib, json, time, subprocess, shutil
from pathlib import Path
from bs4 import BeautifulSoup

WORKSPACE = Path("/sessions/loving-fervent-heisenberg/mnt/corpus")
PDF_DIR = Path("/tmp/b0688_pdfs")
...
TMP_DIR = Path("/tmp/b0688_recover")
