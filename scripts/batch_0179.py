...
Discovery channel /legislation/subsidiary pages 1..10 was exhausted by batch
0178 (pages 11/12 returned 404). Re-scanning the cached discovery HTML (pages
04–10 on disk) surfaces 5 corporate-relevant SIs not in HEAD. One
(2015/033 Road Traffic — Certificates of Security and Insurance Display)
was already deferred in batch 0178 on substance; we defer it again here.

Targets for this batch (4 ingests × 2 fetches = 8 fetches, at MAX_BATCH_SIZE):
  - si/2017/042  Income Tax (Overseas Private Investment Corporation)
                  (Approval and Exemption) Order, 2017
  - si/2017/070  Income Tax (African Management Services Company)
                  (Approval and Exemption) Order, 2017
  - si/2019/025  Income Tax Act (Suspension of tax on payment of interest
                  to non-resident) (Treasury Bill and Bond) Regulations, 2019
  - si/2020/008  Diplomatic Immunities and Privileges (Turkish Cooperation
                  and Coordination Agency) Order, 2020

All are corporate-entity / corporate-tax exemption orders affecting named
companies or bilateral cooperation vehicles.
"""
import hashlib
import io
import json
...
WORKSPACE = "/sessions/friendly-brave-rubin/mnt/corpus"
os.chdir(WORKSPACE)
UTC = timezone.utc
...
def extract_title_and_pdf(html_bytes):
    ...
    for a in soup.find_all("a", href=True):
        ...
        if "/source.pdf" in h or (h.endswith(".pdf") and "/akn/" in h):
            ...
            break
    # Fall back to zambialii media URL pattern
    if not pdf_url:
        for a in soup.find_all("a", href=True):
            h = a["href"]
...
def process_target(session, year, number, head_ids, last_fetch_t, fetch_counter):
    ...
    # Integrity: id collision with HEAD
    if record["id"] in head_ids:
        return f"check1_fail_id_collision:{record['id']}", None, None
    # Integrity: prefix/slot collision within SI namespace
...
def main():
    # Gather HEAD IDs for SI namespace (all SI records already on disk)
    head_ids = set()
    ...
    fetch_counter = [0]
    results = []
    deadline = time.time() + 10 * 60  # internal 10-min soft deadline
    started_at = utc_now()
