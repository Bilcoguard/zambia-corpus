"""Batch 0307 — Phase 4 / minimal upstream-refresh tick (18th consecutive steady-state tick).

Inherited closing state (after b0290..b0306):
  ...
  - OCR backlog: 21 (unchanged from b0293..b0306).
...
PARSER_VERSION: 0.5.0 (probe-only — same as b0290..b0306)
...
PROBE RESULTS (see _work/batch_0307_probe.json for full data):
...
SANDBOX NOTE (carried from b0299..b0306):

  - parliament.gov.zm does not send the RapidSSL TLS RSA CA G1
    intermediate in its TLS chain. The corpus-worker sandbox does not
    have it in its system trust store. Worked around by appending
    ...
    store with the published RapidSSL intermediate).
"""
...
Phase 4 disposition unchanged from b0293..b0306: cannot meaningfully
