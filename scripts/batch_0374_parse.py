#!/usr/bin/env python3
"""Batch 0374 parser-only step (REPARSE-FIRST PASS, continues 2024 ZMCC + 2025 carryover).
...
re-attempted under parser_v0.3.1 + judges_no_comma guard.

Frozen parser baseline scripts/batch_0360_parse.py (parser_v0.3.1,
...
2022-format space-separated judges with
parser_v0.3.1_judges_no_comma_unhandled).
...
Slice: continues the 2024 ZMCC backlog (per b0373 recommendation
"Next reparse tick should pick up zmcc/2024/{16, 15, 13, 11, 10}"),
plus zmcc/2024/23 (still on the legacy generic v0.3.0 reason — never
reclassified under v0.3.1) and two top-of-DESC 2025 carryover
candidates (2025/25 and 2025/24, both still on the legacy generic
v0.3.0 reason). Filling MAX_BATCH_SIZE=8 with addressable v0.3.1
reparse-first candidates per approvals.yaml `reparse_first_note`.

This tick's 8-slice (mixed year-DESC + 2024 num-DESC continuation):
  zmcc 2024/16, 2024/15, 2024/13, 2024/11, 2024/10, 2024/23,
  zmcc 2025/25, 2025/24.
Excluded: 2024/{17, 20, 22, 25, 27} were already reclassified
`html_no_summary_pdf_no_match` under v0.3.1 in b0373 (re-running
v0.3.1 would yield identical deferral). 2024/{2, 4-8} were already
reclassified `html_no_summary_pdf_no_match` under v0.3.1 in b0364
(same rationale). 2025/19 is reclassified
`pdf_extraction_empty_likely_scanned` (needs OCR). 2025 numbers
{1, 3, 4, 13, 20, 23, 26, 27, 29, 30, 31} already have records.

All eight expression dates were verified via grep of the canonical
/eng@YYYY-MM-DD link in each raw HTML before the slice was locked.
...
TARGETS = [
    ...
    # before locking the slice — see worker.log for b0374).
    # Batch 0374 (REPARSE PASS): continues 2024 ZMCC + 2025 carryover.
    # All raw HTML+PDF pairs already on disk from earlier batches;
    # this run consumes zero fetch budget.
    ("zmcc", 2024, 23, "2024-10-29"),
    ("zmcc", 2024, 16, "2024-07-10"),
    ("zmcc", 2024, 15, "2024-07-08"),
    ("zmcc", 2024, 13, "2024-06-28"),
    ("zmcc", 2024, 11, "2024-06-17"),
    ("zmcc", 2024, 10, "2024-06-25"),
