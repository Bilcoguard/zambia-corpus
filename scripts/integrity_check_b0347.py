#!/usr/bin/env python3
"""Integrity check for batch-0347 (Phase 5 ZMCC ingestion).

Validates the 1 record written by batch_0347_parse.py:
...
NEW = ROOT / "records/judgments/zmcc/2025/judgment-zm-2025-zmcc-04-brenda-mpashi-v-attorney-general-and-anor.json"
...
def main():
    ...
    htmls = list(raw_dir.glob("judgment-zm-2025-zmcc-04-*.html"))
    pdfs = list(raw_dir.glob("judgment-zm-2025-zmcc-04-*.pdf"))
    if not htmls or not pdfs:
        errors.append("raw HTML or PDF missing on disk for zmcc-04")
    else:
