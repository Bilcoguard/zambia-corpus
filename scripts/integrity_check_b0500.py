#!/usr/bin/env python3
"""Integrity check for batch-0500 (Phase 5 audit-only zero-yield tick #2).
...
Phase 5 scope, no-regression sweep — matches the b0499 lighter check:
  ...
  - Spot recompute 6 random records (deterministic seed=500) — sha256 must match
...
def main():
    ...
    # spot recompute (seed=500, 6 records)
    if parsed:
        ...
        print(f"spot recompute (seed=500): {spot_ok} / {len(sample)}")
