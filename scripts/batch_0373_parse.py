#!/usr/bin/env python3
"""Batch 0373 parser-only step (REPARSE-FIRST PASS, opens 2024 ZMCC backlog).

Re-parses 8 ZMCC 2024 raw HTML+PDF pairs already on disk that were
...
already on disk under raw/zambialii/judgments/zmcc/2024/.

Slice: opens the 2024 ZMCC raw-on-disk no-record backlog. b0361-b0367
re-attempted 2025 + 2024/{1-8} + 2023 under v0.3.1; b0368-b0372 pivoted
to the 2022 + 2021 backlog (now fully classified). The 2024/{10-22, 25,
27} numbers were deferred under v0.3.0 and never re-attempted under
v0.3.1 — they are the next addressable reparse-first inventory.
b0372's claim that "v0.3.1 reparse inventory for ZMCC is empty" was
strictly scoped to the 2021 sub-backlog; the broader 2024 reparse
inventory remains addressable and is the correct continuation per the
...
This tick takes the DESC top-of-2024 8-slice:
  zmcc 2024/27, 2024/25, 2024/22, 2024/21, 2024/20, 2024/19,
  2024/18, 2024/17.
...
for the next tick's DESC continuation if they remain unaddressed.
...
(see _work/b0373/ slice notes for raw outputs).
...
TARGETS = [
    ...
    # before locking the slice — see worker.log for b0373).
    # Batch 0373 (REPARSE PASS): opens 2024 ZMCC backlog DESC.
    # All raw HTML+PDF pairs already on disk from earlier batches;
    # this run consumes zero fetch budget.
    ("zmcc", 2024, 27, "2024-12-10"),
    ("zmcc", 2024, 25, "2024-11-13"),
    ("zmcc", 2024, 22, "2024-10-15"),
    ("zmcc", 2024, 21, "2024-10-11"),
    ("zmcc", 2024, 20, "2024-10-03"),
    ("zmcc", 2024, 19, "2024-07-26"),
    ("zmcc", 2024, 18, "2024-07-26"),
    ("zmcc", 2024, 17, "2024-07-29"),
]
