#!/usr/bin/env python3
"""
Batch 0163 — Phase 4 bulk ingestion.

Seed + probe pass. Per batch 0162 next-tick plan:

    Seed: Standards Act 1994/20 (deferred from batch 0162 via
    batch cap when probe 'food and drugs' surfaced it).

    Probe rotation: water resources, wildlife, national parks,
    insurance, pensions, securities, telecommunications,
    energy regulation.
...
Mirrors batch 0162 processor logic (HTML/AKN with PDF fallback +
...
# Seed candidate deferred from batch 0162 (batch-cap deferral).
# Surfaced via 'food and drugs' probe in batch 0162.
SEED_CANDIDATES = [
    (1994, 20, "standards-act-1994",
     "batch 0162 deferred via food-and-drugs probe"),
]
...
def write_batch_report(records, gaps, total_sections, fetches, integrity_passed,
    ...
    lines = [
        ...
        "## Strategy",
        "",
        "Seed + probe pass. Stage 1 seeds the Standards Act "
        "1994/20 (deferred from batch 0162 via batch cap after "
        "probe 'food and drugs' surfaced it). Stage 2 probes the "
        "ZambiaLII search API with eight fresh rotation queries "
        "per the batch-0162 next-tick plan: water resources, "
        "wildlife, national parks, insurance, pensions, "
        "securities, telecommunications, energy regulation. "
        "Hits surviving HEAD + title filters fill slots up to "
    ...
    lines.extend([
        "## Notes",
        "",
        "- SEED stage includes Standards Act 1994/20 (deferred "
        "from batch 0162 via batch cap after 'food and drugs' "
        "probe).",
        ...
        "- Next tick: if probe yield this batch was <= 2 new "
        "primary parents, shift to the alphabetical "
        "`/akn/zm/act/` listing traversal fallback for "
        "unresolved Cap. parents; otherwise continue the "
        "primary-statute sweep with another fresh rotation of "
        "narrower probe keywords — water, environment, "
        "communications, roads, traffic, lands, petroleum, "
        "banking.",
        "",
    ])
