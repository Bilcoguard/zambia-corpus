...
Per batch-0194 next-tick plan: continue sis_family with
si/2016/8 Anti-Gender Based Violence (Court) Rules, 2016
(hand-identified from alphabet=A cache). Follow with a single
alphabet=G discovery fetch to find additional sis_family
candidates (Guardianship, Gender-Based Violence, General
Marriage orders) for next-tick work. Deliberately small batch
(1 ingest + 1 discovery = 3 fetches) to stay well inside the
20-minute tick budget on 5-6s crawl-delay.

Robots.txt re-verified at tick start (2026-04-24):
  /akn/zm/judgment/ and /akn/zm/officialGazette/ still Disallowed
  for User-agent: *. All targets this tick are under
  /akn/zm/act/si/ and /legislation/subsidiary — both permitted.
  Content-Signal directive note (ai-train=no, ai-input=no) is a
  non-technical reservation of rights under EU DSM Art. 4; flagged
  in worker.log for host review, worker continues under existing
  pattern.

sis_family currently holds 1 record in HEAD after batch-0194
(si-zm-2023-038 Intestate Succession Rules). This batch pushes it
to 2.
"""
import argparse
import hashlib
...
TARGETS = [
    (2016, 8, "sis_family"),   # Anti-Gender Based Violence (Court) Rules, 2016
]

DISCOVERY_URL = "https://zambialii.org/legislation/subsidiary?alphabet=G"
...
FAMILY_KEYWORDS = [
    "gender", "gbv", "guardian", "juvenile", "adopt", "affili",
    "marriage", "matrimonial", "child", "intest", "succession",
    "paternity", "custody", "maintenance of",
    "family", "ward", "minor", "orphan", "foster",
]


def discovery_fetch(session, last_fetch_t, fetch_counter):
    """Fetch alphabet=G discovery page and extract candidate SIs for
    ...
    path = f"_work/batch_{BATCH_NUM}_alphabet_G.html"
    ...
    with open("provenance.log", "a") as f:
        f.write(json.dumps({"request_url": url, "status": 200,
                            ...
                            "parser_version": PARSER_VERSION,
                            "role": "discovery"}) + "\n")
    # Extract family-related candidates from this page
    ...
    for a in soup.select("a[href*='/akn/zm/act/si/']"):
        ...
        seen.add(slug)
        low = t.lower()
        if any(k in low for k in FAMILY_KEYWORDS):
...
def main():
    ...
    summary = {
        "batch": BATCH_NUM,
        "phase": "phase_4_bulk",
        "sub_phase": "sis_family_continuation",
