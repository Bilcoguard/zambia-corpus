#!/usr/bin/env python3
"""
Batch 0157 — Phase 4 bulk ingestion.

Continuation of the batch 0156 next-tick plan. Probe-only pass — no
deferred SEED queue carries forward from batch 0156. ZambiaLII
search-API queries rotate to a fresh set of primary-parent targets
flagged unresolved by the batch-0156 wrap-up: arbitration (primary
2000), copyright (Cap. 406 parent), higher education (pre-2013),
electoral (pre-2016), Zambia Revenue Authority (1993 parent),
lands tribunal, mental disorders (synonym probe for mental-health
parent), and refugees control (2017 parent) — the 2017 Refugees Act
repeals the 1970 Refugees (Control) Act ingested in batch 0155.

Hits surviving HEAD + title filters fill slots up to
MAX_RECORDS=8. Title filter rejects any slot whose AKN-page title
contains 'amendment' (including the OCR variants 'amendrnent' and
'amendement'), 'appropriation', 'repeal', 'supplementary',
'validation', or 'transitional' — applied pre-write.

Mirrors batch 0156 processor logic (HTML/AKN with PDF fallback +
pre-write B-POL-ACT-1 title filter). If the 2017 Refugees Act
surfaces and is committed, a repeal-chain link from the 1970
Refugees (Control) Act to the new 2017 record is applied after the
...
# No seed candidates this batch — batch-0156 probe-only run left no
# deferred queue behind; all probe candidates were either rejected
# pre-fetch or ingested.
SEED_CANDIDATES = []
...
# Repeal-chain link applied ONLY if the 2017 Refugees Act record is
# committed this batch. Resolved at runtime after Stage 2.
REFUGEES_1970_PATH = os.path.join(
    "records", "acts", "1970",
    "act-zm-1970-040-refugees-control-act-1970.json",
)
REFUGEES_2017_CAND_PREFIX = "act-zm-2017-"
REFUGEES_2017_TITLE_HINT = "refugees"
...
def resolve_refugees_repeal(records):
    """If a 2017 Refugees Act record was committed, return a repeal-chain
    link from the 1970 Refugees (Control) Act to that new record. Returns
    an empty list otherwise. The 2017 Refugees Act (No. 1 of 2017) at
    section 64 repealed the 1970 Refugees (Control) Act Cap. 120."""
    if not os.path.exists(os.path.join(WORKSPACE, REFUGEES_1970_PATH)):
        return []
    for r in records:
        if not r["id"].startswith(REFUGEES_2017_CAND_PREFIX):
            continue
        if REFUGEES_2017_TITLE_HINT not in r["title"].lower():
            continue
        note = (
            "Refugees Act, 2017 s.64 repeals the Refugees (Control) "
            "Act, 1970 (Cap. 120). Link applied to the 1970 record."
        )
        return [(REFUGEES_1970_PATH, r["id"], note)]
    return []
...
def write_batch_report(records, gaps, total_sections, fetches, integrity_passed,
    ...
    lines = [
        ...
        "## Strategy",
        "",
        "Probe-only pass — batch-0156 left no deferred SEED queue "
        "behind. Stage 2 probes the ZambiaLII search API with eight "
        "queries targeting fresh primary-parent targets flagged "
        "unresolved by the batch-0156 wrap-up: arbitration (primary "
        "2000), copyright (Cap. 406 parent), higher education "
        "(pre-2013), electoral (pre-2016), Zambia Revenue Authority "
        "(1993 parent), lands tribunal, mental disorders (synonym "
        "for mental-health parent), and refugees (2017 repealer of "
        "the 1970 Refugees (Control) Act ingested batch 0155). Hits "
        "surviving HEAD + title filters fill slots up to "
        f"MAX_RECORDS={MAX_RECORDS}. Title filter rejects any slot "
        "whose AKN-page title contains `amendment` (plus OCR "
        "variants `amendrnent` and `amendement`), `appropriation`, "
    ...
    if repeal_changes:
        ...
        lines.append(
            "No new repeal-chain links applied this batch — the "
            "conditional Refugees 1970 -> 2017 link was not "
            "triggered (no 2017 Refugees Act record was committed "
            "this batch)."
        )
    lines.extend([
        "",
    ...
    lines.extend([
        "## Notes",
        "",
        "- No SEED stage this batch — batch-0156 was a probe-only "
        "pass that left no deferred probe queue behind.",
        "- B-POL-ACT-1 title filter extended this batch to include "
        "the OCR variants `amendrnent` (rn->m digraph, per batch "
        "0156 diagnostic) and `amendement` (stray 'e'). This should "
        "prevent the 2000/18 'Banking and Financial Services "
        "(Amendrnent) Act' style slip-through observed in batch 0156.",
        "- Conditional repeal-chain link: if the 2017 Refugees Act "
        "surfaces as a probe hit and is committed, a repeal link "
        "from `act-zm-1970-040-refugees-control-act-1970` to the "
        "2017 record is applied after Stage 2. This reflects the "
        "2017 Refugees Act s.64 repeal-and-savings clause.",
        "- Next tick: continue primary-statute sweep with the next "
        "rotation of probe keywords — extradition, treason, "
        "immigration (pre-1965 parent, Cap. 123 if not already in "
        "HEAD), arms-and-ammunition, aviation, industrial-relations "
        "(pre-1993 parent), money-laundering (pre-2001 parent), and "
        "Patents/Copyright Cap. parents if they have not surfaced "
        "by alphabetical-listing browse. Patents Cap. 400 and any "
        "pre-independence ordinances may require a direct /akn "
        "alphabetical listing fallback rather than keyword probes.",
        "",
    ])
...
def main():
    ...
    # Conditional repeal-chain link: Refugees 1970 -> 2017 (if the 2017
    # record surfaced and was committed this batch). Head-ids updated
    # with this batch's committed IDs so CHECK 4 resolves against the
    # about-to-be-committed state.
    post_head_ids = set(head_ids) | set(r["id"] for r in records)
    dynamic_links = resolve_refugees_repeal(records)
