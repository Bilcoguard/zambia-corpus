#!/usr/bin/env python3
"""
Batch 0158 — Phase 4 bulk ingestion.

Continuation of the batch 0157 next-tick plan. Pre-write HEAD audit
(git ls-tree HEAD records/acts/) confirmed that six of the primary-
parent targets flagged by batch 0157 (Arbitration 2000/19, Copyright
1994/44, Higher Education 2013/4, Lands Tribunal 2010/39, Mental
Disorders 1949/21, Refugees Act 2017/1) are ALREADY in HEAD and
therefore do not enter this batch's seed queue.

The 2017 Refugees Act is present under id 'act-zm-2017-001-refugees',
but the 1970 Refugees (Control) Act (act-zm-1970-040-refugees-control-
act-1970) still has repealed_by=null, so this batch applies an
UNCONDITIONAL repeal-chain link from the 1970 record to the 2017
record. This variant of the resolver used in batch 0157 does not
depend on a same-batch commit — the target is already in HEAD, so
CHECK 4 resolves immediately.

Probe-only ingestion pass. Rotates ZambiaLII search-API queries to
the next set flagged by batch 0157: treason, immigration, arms and
ammunition, aviation, industrial relations, money laundering, sale
of goods, bills of exchange. Hits surviving HEAD + title filters
fill slots up to MAX_RECORDS=8.

Title filter rejects any slot whose AKN-page title contains
...
Mirrors batch 0157 processor logic (HTML/AKN with PDF fallback +
pre-write B-POL-ACT-1 title filter + OCR variants). Wall-clock
guarded to stay under ~16 min of Stage 2 work to leave margin for
...
# No seed candidates this batch — the primary parents flagged by
# batch 0157 next-tick were all confirmed already-in-HEAD by a
# pre-write ls-tree audit.
SEED_CANDIDATES = []
...
# UNCONDITIONAL repeal-chain link for this batch: the 2017 Refugees
# Act (act-zm-2017-001-refugees) is already in HEAD (ingested in an
# earlier batch) but the 1970 Refugees (Control) Act still has
# repealed_by=null. Refugees Act 2017 s.64 repeals and replaces the
# 1970 Refugees (Control) Act (Cap. 120), so we apply that link
# here. Target is verified against HEAD — not dependent on this
# batch's new commits.
UNCONDITIONAL_REPEAL_LINKS = [
    (
        os.path.join(
            "records", "acts", "1970",
            "act-zm-1970-040-refugees-control-act-1970.json",
        ),
        "act-zm-2017-001-refugees",
        "Refugees Act, 2017 s.64 repeals the Refugees (Control) "
        "Act, 1970 (Cap. 120). 2017 Act verified in HEAD by pre-"
        "write ls-tree audit; target id 'act-zm-2017-001-refugees' "
        "confirmed.",
    ),
]
...
def write_batch_report(records, gaps, total_sections, fetches, integrity_passed,
    ...
    lines = [
        ...
        "## Strategy",
        "",
        "Probe-only pass. A pre-write HEAD audit "
        "(`git ls-tree HEAD records/acts/`) confirmed that the six "
        "primary-parent targets flagged by the batch-0157 next-tick "
        "plan (Arbitration 2000/19, Copyright 1994/44, Higher "
        "Education 2013/4, Lands Tribunal 2010/39, Mental Disorders "
        "1949/21, Refugees 2017/1) are already in HEAD — so none "
        "of them enter this batch's seed queue. Stage 2 probes the "
        "ZambiaLII search API with eight fresh rotation queries: "
        "treason, immigration, arms and ammunition, aviation, "
        "industrial relations, money laundering, sale of goods, "
        "bills of exchange. Hits surviving HEAD + title filters "
        f"fill slots up to MAX_RECORDS={MAX_RECORDS}. Title filter "
        "rejects any slot whose AKN-page title contains `amendment` "
        "(plus OCR variants `amendrnent` and `amendement`), "
        "`appropriation`, `repeal`, `supplementary`, `validation`, "
        "or `transitional` — applied pre-write, so rejected slots "
        "produce no raw or record file. PDF fallback is invoked "
        "only when the HTML returns fewer than 2 parsed sections. "
        "Additionally, this batch applies one unconditional "
        "repeal-chain link (Refugees 1970 -> 2017) against a "
        "target already in HEAD.",
        "",
        "## Committed records",
        "",
    ...
    if repeal_changes:
        ...
        lines.append(
            ...
            "pre-declared unconditional link list was empty or "
            "skipped (target not in HEAD / source record missing)."
        )
    lines.extend([
        "",
        "## Seed summary",
    ...
    lines.extend([
        "## Notes",
        "",
        "- No SEED stage this batch — pre-write ls-tree audit "
        "confirmed all six batch-0157 next-tick primary-parent "
        "targets (Arbitration 2000/19, Copyright 1994/44, Higher "
        "Education 2013/4, Lands Tribunal 2010/39, Mental Disorders "
        "1949/21, Refugees 2017/1) are already in HEAD.",
        "- B-POL-ACT-1 title filter retains the OCR variants "
        "`amendrnent` and `amendement` added in batch 0157 to guard "
        "against the 2000/18 'Banking and Financial Services "
        "(Amendrnent) Act' style slip-through observed in batch 0156.",
        "- Unconditional repeal-chain link applied: "
        "`act-zm-1970-040-refugees-control-act-1970` -> "
        "`act-zm-2017-001-refugees`. The 2017 Refugees Act "
        "(Act No. 1 of 2017) at section 64 repeals the Refugees "
        "(Control) Act 1970 (Cap. 120); target verified in HEAD by "
        "pre-write ls-tree audit.",
        "- Next tick: continue primary-statute sweep with a fresh "
        "rotation of probe keywords — extradition (already in HEAD "
        "1968/47 — skip), treason, hire purchase, stamp duty, "
        "juveniles (Cap. 53), Patents Cap. 400, road traffic "
        "pre-2002, immigration pre-1965 (Cap. 123) if not already "
        "in HEAD, Zambia Institute of Purchasing and Supply siblings. "
        "Patents Cap. 400 and any pre-independence ordinances may "
        "require a direct /akn alphabetical-listing fallback rather "
        "than keyword probes.",
        "",
    ])
...
def main():
    ...
    # Unconditional repeal-chain link(s): target must be in HEAD (or
    # committed this batch) to resolve. The Refugees 1970 -> 2017
    # target is already in HEAD per pre-write audit.
