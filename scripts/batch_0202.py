#!/usr/bin/env python3
"""
Batch 0202 — Phase 4 sis_corporate continuation.

Per batch-0201 next-tick plan:
  "continue sis_corporate next tick, with these probes:
     - alphabet=B re-scan for any BFS Regulations missed (BFS Capital
       Adequacy, BFS Disclosure of Lending Rates, BFS Liquidity)
     - PACRA Act 2010/15 derivatives via alphabet=P
     - Securities Act 2016/41 derivatives via alphabet=S
     - Money Lenders Act / Building Societies Act SIs (alphabet=M, B)
   Re-verify robots.txt at start of next tick."

Workflow:
  1. Re-verify robots.txt (sha256, must match prefix fce67b697ee4ef44).
  2. Live discovery probes — alphabets B, M, P, S — extract /akn/zm/act/si/
     hrefs along with their visible link text for tagging.
  3. Filter to novel (year, number) slots not already in HEAD records/sis,
     then keep only candidates whose visible title contains corporate /
     finance / banking / securities / building-society / money-lenders /
     PACRA / patents / pension keywords.
  4. Ingest up to MAX_BATCH_SIZE = 8 records using the same path that
     batch_0201 followed (HTML → PDF → pdfplumber sectioning → CHECK1-5).
...
def main():
    ...
    a.add_argument("--alphabets", default="BMPS")
