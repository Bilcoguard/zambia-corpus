#!/usr/bin/env python3
"""Batch 0370 parser-only step (REPARSE-FIRST PASS, continuation of b0369).
...
Slice: continuation of the 2022 ZMCC backlog. b0369 took DESC slice
{23,22,21,20,18,17,16,15} (0 written / 8 deferred — judges_no_comma
plus html_no_summary_pdf_no_match dominated). This batch continues
year-DESC then num-DESC into the next eight raw-on-disk no-record
candidates per b0369 next-tick recommendation (a):
  zmcc 2022/14, 2022/13, 2022/12, 2022/11, 2022/10, 2022/9,
  2022/8, 2022/6.
(zmcc/2022/7 has no raw on disk — skipped in DESC walk.)
...
TARGETS = [
    ...
    # before locking the slice — see worker.log for b0370).
    # Batch 0370 (REPARSE PASS): continuation of b0369 2022 ZMCC sweep.
    # b0369 took {23,22,21,20,18,17,16,15} (0 written / 8 deferred).
    # This batch takes the next eight raw-on-disk no-record candidates
    # (year-DESC then num-DESC) per b0369 next-tick plan (recommendation a).
    # zmcc/2022/7 has no raw on disk — skipped in the DESC walk.
    # All raw HTML+PDF pairs already on disk from earlier batches;
    # this run consumes zero fetch budget.
    ("zmcc", 2022, 14, "2022-08-03"),
    ("zmcc", 2022, 13, "2022-07-28"),
    ("zmcc", 2022, 12, "2022-06-20"),
    ("zmcc", 2022, 11, "2022-05-16"),
    ("zmcc", 2022, 10, "2022-05-19"),
    ("zmcc", 2022,  9, "2022-03-14"),
    ("zmcc", 2022,  8, "2022-04-13"),
    ("zmcc", 2022,  6, "2022-02-24"),
]
