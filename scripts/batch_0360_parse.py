#!/usr/bin/env python3
"""Batch 0360 parser-only step.

Parses the 8 ZMCC raw HTML+PDF pairs persisted by batch_0359_fetch.py
under the tightened parser_version 0.3.0 policy locked in at b0344
(frozen from b0358 — byte-identical except for the TARGETS slice and
WORK directory).

Slice: ZMCC 2022/{3,2,1} (final 3 candidates of 2022) plus the start
of the ZMCC 2021 DESC sweep at 2021/{22,21,20,19,18}. Top of 2021 is
2021/24 (2021/25 confirmed 404 upstream); 2021/{24,23} remain to be
fetched in a subsequent tick.
...
TARGETS = [
    ...
    # in the persisted raw HTML.
    # Batch 0360: ZMCC 2021/{24,23} (top of year, fresh) +
    # ZMCC 2021/{17,16,15,14,13,12} (continuation DESC).
    # 2021/{18,19,21,22} were deferred in earlier ticks under
    # parser_v0.3.0 — they are reserved for a future re-parse pass per
    # 2026-04-30 user instruction (gaps.md reprocessing).
    ("zmcc", 2021, 24, "2021-10-27"),
    ("zmcc", 2021, 23, "2021-11-29"),
    ("zmcc", 2021, 17, "2021-09-20"),
    ("zmcc", 2021, 16, "2021-11-22"),
    ("zmcc", 2021, 15, "2021-09-17"),
    ("zmcc", 2021, 14, "2021-07-13"),
    ("zmcc", 2021, 13, "2021-07-20"),
