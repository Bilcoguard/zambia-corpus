#!/usr/bin/env python3
"""Batch 0375 parser-only step (REPARSE-FIRST PASS, 2025 ZMCC continuation).
...
already on disk under raw/zambialii/judgments/zmcc/2025/.

Slice: continues the 2025 ZMCC raw-on-disk no-record backlog per
b0374 NOTE recommendation: "Next reparse tick should pick up
zmcc/2025/{22, 21, 18, 17, 16, 15, 14, 12, 11}". Taking the first
8 in DESC order; zmcc/2025/11 rolls over to the next tick because
MAX_BATCH_SIZE=8.

This tick's 8-slice (2025 ZMCC num-DESC continuation):
  zmcc 2025/22, 2025/21, 2025/18, 2025/17, 2025/16, 2025/15,
  2025/14, 2025/12.

Excluded: 2025/{1, 3, 4, 13, 20, 23, 26, 27, 29, 30, 31} already
have records on disk. 2025/{2, 5, 6, 7, 8, 9, 10, 19, 24, 25, 28}
sit on legacy v0.3.0 deferrals or earlier-batch reclassifications
that will be picked up by subsequent reparse ticks; 2025/11 rolls
over to b0376 as MAX_BATCH_SIZE=8 fills the slice.
...
TARGETS = [
    ...
    # before locking the slice — see worker.log for b0375).
    # Batch 0375 (REPARSE PASS): 2025 ZMCC num-DESC continuation.
    ...
    ("zmcc", 2025, 18, "2025-09-30"),
    ("zmcc", 2025, 17, "2025-08-27"),
    ("zmcc", 2025, 16, "2025-08-25"),
    ("zmcc", 2025, 15, "2025-07-23"),
    ("zmcc", 2025, 14, "2025-07-28"),
    ("zmcc", 2025, 12, "2025-06-27"),
]
