"""Batch 0338 — Phase 4 / minimal upstream-refresh tick (49th consecutive steady-state tick).

Inherited closing state (after b0290..b0337):
  ...
  - OCR backlog: 21 (unchanged from b0293..b0337).
...
PARSER_VERSION: 0.5.0 (probe-only — same as b0290..b0337)
...
PROBE RESULTS (see _work/batch_0338_probe.json for full data):
  ...
  /legislation/recent:    15 act links (2026/1..11 + 2025/6..9).
                          ...
                          (UNCHANGED for the tenth consecutive tick —
                          feed surface and ranking are byte-for-byte
                          identical to b0329..b0337).
  parliament page 0:      20 act links (2026/1..11 + 2025/21..29).
                          ALL 20 in corpus.
                          page bytes: 38,208 (unchanged across all 49
                          steady-state ticks).
                          page sha256: 76246fb01b3dc6abe3c655e9c4593e0980b631fcb697cb6f742c0ecefef24edb
...
CROSS-CHECK NOTE:
  ...
  The probe's in-corpus check uses a full-tree walk over
  records/acts/**/act-zm-{yr}-0*{num}-*.json (the records/ tree mixes
  a flat top-level for acts predominantly indexed since 2025 and per-
  year subdirectories for the historical sweep). Year/num pairs match
  whether the file lives at records/acts/<yr>/ or at records/acts/.
  This tick the probe was rebuilt under a fresh sandbox session —
  ROOT path was rewritten from the prior session's mount to the
  current session mount, and the in-corpus check was hardened to
  walk the flat top-level explicitly (the b0337 cross-check note
  documented this behaviour but the inline implementation only
  inspected records/acts/<yr>/; behaviour is now aligned with
  the documented contract).

SANDBOX NOTE (carried from b0299..b0337):
...
# The probe lives at _work/batch_0338_probe.py (gitignored scratch).
PICKS = []
"""
...
Phase 4 disposition unchanged from b0293..b0337: cannot meaningfully
