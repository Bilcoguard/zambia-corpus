#!/usr/bin/env python3
"""
Batch 0133 — Phase 4 bulk ingestion.
Targets: 8 Acts (O/P block + oaths + lotteries) from ZambiaLII direct AKN URLs.
"""
...
# (year, act_number, short_title_hint_for_slug)
TARGETS = [
    (1938, 44, "commissioners-for-oaths-act-1938"),
    (1990, 4, "official-oaths-act-1990"),
    (2007, 18, "plant-breeders-rights-act-2007"),
    (2010, 36, "occupational-health-and-safety-act-2010"),
    (1953, 15, "probation-of-offenders-act-1953"),
    (1936, 22, "probates-resealing-act-1936"),
    (1940, 38, "pharmacy-and-poisons-act-1940"),
    (1970, 7, "state-lotteries-act-1970"),
]
...
def process_act(year, number, slug_hint):
    ...
    if r["status"] != 200 or r["body_len"] < 1000:
        return None, f"HTML fetch failed: status={r['status']} len={r['body_len']}"
    ...
    # Try PDF fallback for empty/sparse HTML
    pdf_body = None
    pdf_hash = None
    pdf_url_used = None
    if len(sections) < 2:
        pdf_url = source_url.rstrip("/") + "/source.pdf"
        print(f"  HTML empty, trying PDF: {pdf_url}")
        pr = do_fetch(pdf_url)
        rate_limit()
    ...
    # Normalise: add "Act, YYYY" suffix if missing
    # Save raw bytes under raw/zambialii/<year>/<record-id>.{html|pdf}
    # We need the record id first; build preliminary
    ...
    # Save record JSON to records/acts/<year>/<id>.json
...
def integrity_check_batch(records, raw_map):
    errors = []
    ...
    # 3. source_hash matches raw file on disk
    for r in records:
        src_hash = r["source_hash"].replace("sha256:", "")
        raw_file = raw_map.get(r["id"])
        if not raw_file or not os.path.exists(raw_file):
            errors.append(f"RAW MISSING: {r['id']}")
            continue
        with open(raw_file, "rb") as f:
            fh = hashlib.sha256(f.read()).hexdigest()
        if fh != src_hash:
            errors.append(f"HASH MISMATCH: {r['id']}")

    # 4. amended_by / repealed_by references (empty in this batch) — trivially pass
...
def write_batch_report(records, gaps, total_sections, fetches, integrity_passed):
    ...
    for i, r in enumerate(records, 1):
        src = "PDF" if "source.pdf" in r.get("source_url", "") else "HTML/AKN"
        lines.append(
    ...
    lines.extend([
        ...
        "",
        "## Integrity checks",
        "- CHECK 1 (batch unique IDs): " + ("PASS" if integrity_passed else "see worker.log"),
        "- CHECK 2 (no HEAD collision): see worker.log",
        "- CHECK 3 (source_hash matches raw on disk): see worker.log",
        "- CHECK 4 (amended_by / repealed_by reference resolution): PASS — no cross-refs in this batch",
        "- CHECK 5 (required fields present): see worker.log",
        "",
        "## Raw snapshots",
        "",
    ...
    lines.extend([
        "## Notes",
        "",
        "Targets drawn from batch 0132's next-tick list (O/P-block Acts) plus two additional oaths/lotteries Acts.",
        "",
    ])
...
def main():
    ...
    for year, num, hint in TARGETS:
        try:
            record, err = process_act(year, num, hint)
        except Exception as e:
            err = f"Exception: {type(e).__name__}: {e}"
            record = None
        if err or record is None:
            gaps.append(f"Act {num} of {year} ({hint}): {err}")
            continue
        records.append(record)
        # Find raw file path for integrity check
        raw_subdir = os.path.join(RAW_DIR, str(year))
        # Prefer pdf if source_url ends in source.pdf, else html
        if "source.pdf" in record["source_url"]:
            raw_map[record["id"]] = os.path.join(raw_subdir, f"{record['id']}.pdf")
        else:
            raw_map[record["id"]] = os.path.join(raw_subdir, f"{record['id']}.html")

    # Integrity
    errors = integrity_check_batch(records, raw_map)
    ...
    with open(WORKER_LOG, "a") as f:
        ...
        f.write(
            f"{ts} Phase 4 Batch {BATCH_NUM} COMPLETE: +{len(records)} Acts via ZambiaLII "
            f"HTML/PDF. Records: {titles}. {total_sections} sections total. "
            f"Fetches: {fetch_count}. Integrity: ALL PASS. Gaps: {len(gaps)}. "
            f"Report: reports/batch-{BATCH_NUM}.md. B2 sync skipped — rclone not available.\n"
        )

    print(f"=== Batch {BATCH_NUM} done: {len(records)} records, {fetch_count} fetches ===")
    print(f"Gaps: {gaps}")
