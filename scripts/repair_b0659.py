#!/usr/bin/env python3
"""
Repair worker tick b0659.

Continues the ZambiaLII SI no-body drain.  Targets up to MAX_BATCH_SIZE records
with NULL/empty bodies (or corrupted/stub bodies per v4 prompt).  Quality-gated,
crash-safe per-record commit.

Adapted from scripts/repair_b0657.py (path changes only).
"""
...
ROOT = Path("/sessions/wizardly-tender-thompson/mnt/corpus")
WORK_DB = Path("/tmp/corpus_work_b0659.sqlite")  # tmpfs copy to dodge virtiofs journal quirks
os.chdir(ROOT)

TICK = "b0659"
PARSER_VERSION = "repair-0.6.3"
MAX_BATCH_SIZE = 8
WALL_CLOCK_BUDGET_S = 60 * 18  # 2 min headroom inside 20-min limit
...
def main() -> int:
    ...
    report_path = ROOT / "reports" / f"repair-batch-{TICK}.md"
    lines = [
        f"# Repair batch {TICK} — {fixed} record(s) fixed, {failed} failed",
        "",
