#!/usr/bin/env python3
"""
Batch 0194 — Phase 4 sis_mining closeout + sis_family rotation opener.

Per batch-0193 next-tick plan: sis_mining yield has thinned — alphabet=M
listing now holds only 1 novel mining SI (si/2018/82 Mining Appeals
Tribunal Rules, 2018); all other mining SIs on that listing are already
in HEAD. Per the "yield thins below 3 -> rotate to sis_family" rule, we
close out sis_mining with si/2018/82 and open sis_family.

sis_family currently holds 0 SI records in HEAD (pure cold-start). Family
Acts are present (Marriage Act 1918, Matrimonial Causes Act 2007,
Children's Code Act 2022, Affiliation and Maintenance 1995, Intestate
Succession 1989) but no corresponding SIs have been ingested yet.

Cached alphabet=I discovery from batch-0182 has identified
si/2023/38 "Intestate Succession Rules, 2023" as a clean, titled family
SI candidate. That is the sis_family opener this tick.

Robots.txt recheck: zambialii.org/robots.txt re-fetched 2026-04-24.
Disallowed paths remain /akn/zm/judgment/ and /akn/zm/officialGazette/.
Legislation paths /akn/zm/act/ remain allowed. Worker UA matches
User-agent: * (Crawl-delay 5s). All targets this tick are under
/akn/zm/act/si/ which is allowed.

Targets (2 ingests x 2 fetches = 4 AKN HTML + PDF) + 1 discovery fetch
= 5 fetches:
  - si/2018/82 Mining Appeals (Tribunal) Rules, 2018   (sis_mining closeout)
  - si/2023/38 Intestate Succession Rules, 2023        (sis_family opener)
  - DISCOVERY alphabet=A  (for next-tick sis_family candidates —
                            Affiliation, Adoption, Administration of Estates)

Deliberately conservative: 2 ingests + 1 discovery fits well inside the
20-minute tick budget even on the worst-case 5s crawl-delay path.
Next tick will widen sis_family with candidates drawn from the
alphabet=A cache committed by this batch.
"""
import argparse
import hashlib
import io
...
TARGETS = [
    (2018, 82, "sis_mining"),   # Mining Appeals (Tribunal) Rules, 2018
    (2023, 38, "sis_family"),   # Intestate Succession Rules, 2023
]

DISCOVERY_URL = "https://zambialii.org/legislation/subsidiary?alphabet=A"
...
def discovery_fetch(session, last_fetch_t, fetch_counter):
    """Fetch alphabet=A discovery page and extract candidate SIs for
    sis_family next-tick work. Not ingested; cached to _work/."""
    url = DISCOVERY_URL
    started = utc_now()
    try:
        r = fetch(url, session, last_fetch_t)
    except Exception as e:
        return {"status": f"fetch_error:{type(e).__name__}:{e}", "url": url}
    fetch_counter[0] += 1
    ...
    path = f"_work/batch_{BATCH_NUM}_alphabet_A.html"
    ...
    with open("provenance.log", "a") as f:
        f.write(json.dumps({"request_url": url, "status": 200,
                            ...
                            "parser_version": PARSER_VERSION,
                            "role": "discovery"}) + "\n")
    # Extract family-related candidates
    soup = BeautifulSoup(body, "html.parser")
    seen = set()
    candidates = []
    for a in soup.select("a[href*='/akn/zm/act/si/']"):
        t = a.get_text(" ", strip=True)
        href = a.get("href", "")
        slug = href.split("/eng")[0]
        if slug in seen:
            continue
        seen.add(slug)
        low = t.lower()
        if any(k in low for k in ["adopt", "affili", "administration of estate",
                                   "administrator-general", "adminstrator general"]):
            m = re.match(r"/akn/zm/act/si/(\d+)/(\d+)", slug)
            if m:
                candidates.append({
                    "year": int(m.group(1)),
    ...
    cand_path = f"_work/batch_{BATCH_NUM}_candidates.json"
    with open(cand_path, "w") as f:
        json.dump(candidates, f, indent=2)
    return {
        "status": "ok", "url": url, "sha256": h, "bytes": len(body),
        "cached_at": path, "candidates_found": len(candidates),
...
def main():
    ...
    if not args.skip_discovery:
        ...
        discovery = discovery_fetch(session, last_fetch_t, fetch_counter)
    ...
    summary = {
        "batch": BATCH_NUM,
        "phase": "phase_4_bulk",
        "sub_phase": "sis_mining_closeout+sis_family_opener",
