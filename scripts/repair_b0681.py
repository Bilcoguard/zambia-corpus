#!/usr/bin/env python3
"""b0681 repair worker — ZambiaLII SI repair (no-body acts/SIs continuation).

Drains the 105 acts/SIs with no body. Uses /tmp staging DB to avoid
virtiofs write-back I/O errors on the workspace volume (91% capacity).

Modeled on repair_b0667.py established pattern. Parser version: repair-0.6.81.
"""
import sqlite3, urllib.request, urllib.error, ssl, re, os, sys, hashlib, json, time, subprocess, shutil
from pathlib import Path
from bs4 import BeautifulSoup

WORKSPACE = Path("/sessions/relaxed-nifty-johnson/mnt/corpus")
PDF_DIR = Path("/tmp/b0681_pdfs")
...
TMP_DIR = Path("/tmp/b0681_recover")
...
def main():
    ...
    Path("/tmp/b0681_recover/result.json").write_text(json.dumps(result, indent=2))
