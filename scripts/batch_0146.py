#!/usr/bin/env python3
"""
Batch 0146 — Phase 4 bulk ingestion.

Fresh discovery via ZambiaLII search API (results_html parse) for new
subject areas identified in the batch 0145 next-tick plan: Narcotic
Drugs and Psychotropic Substances Act (1993/37), Bank of Zambia Act
(1996/43 parent), Anti-Corruption Act (1996/42), Citizens Economic
Empowerment Act (2006/9), Probate and Administration of Estates
(1960/41), Rent Act parent (1972/10), Food and Drugs Act (1972/22),
Arbitration 1995/23, plus buffer candidates from Children / Mines /
Road Traffic / Customs searches (to be rejected by title filter if
amendments).

Candidate (year, num) pairs were obtained from the `results_html`
fragment of the search API
(`/search/api/documents/?search=<q>&nature=Act`) for queries
including 'Narcotic Drugs', 'Bank of Zambia', 'Anti-Corruption',
'Citizens Economic Empowerment', 'Arbitration', 'Probate', 'Rent',
'Food and Drugs', 'Children Act', 'Mines and Minerals',
'Road Traffic', 'Customs and Excise'. Each hit was cross-referenced
against HEAD by (year, num) tuple via
`git ls-tree -r HEAD records/acts/`. Probes logged to costs.log /
provenance.log under batch tag "0146-probe".
...
# priority: principal modern statutes first, then older parents.
TARGETS = [
    (1993, 37, "narcotic-drugs-psychotropic-1993"),
    (1996, 43, "bank-of-zambia-1996"),
    (1996, 42, "anti-corruption-1996"),
    (2006,  9, "citizens-economic-empowerment-2006"),
    (1960, 41, "probate-administration-estates-1960"),
    (1972, 22, "food-and-drugs-1972"),
    (1972, 10, "rent-act-1972"),
    (1995, 23, "arbitration-1995"),
    (1989, 14, "children-candidate-1989"),
    (1995,  5, "children-candidate-1995"),
    (2001, 11, "bank-of-zambia-2001"),
    (2011, 28, "mines-minerals-2011"),
]
...
def write_batch_report(records, gaps, total_sections, fetches, integrity_passed):
    ...
    lines = [
        ...
        "## Strategy",
        "",
        "Continuation of the batch 0144 next-tick plan: Lands Act parent "
        "(1995), Juveniles Act parent, Defence Act parent, Banking and "
        "Financial Services Act parent (pre-2017), Securities Act parent, "
        "Public Finance Management Act, plus additional "
        "National Parks and Wildlife / Banking / Juveniles candidates. "
        "Candidate (year, num) pairs were obtained by parsing the "
        "`results_html` fragment of the ZambiaLII search API "
        "(`/search/api/documents/?search=<q>&nature=Act`) for eight "
        "hint queries; each hit was cross-referenced against HEAD by "
        "(year, num) tuple via `git ls-tree -r HEAD records/acts/`. "
        "Ten non-HEAD candidates were queued; the processor fetches each "
    ...
    lines.extend([
        ...
        "candidates include the Securities Act 1993 / Securities Act 2016 "
        "(if distinct), Defence Act subsidiary/parent variants, and the "
        "pre-1964 Juveniles / Protection of Children Act parent; plus "
        "probes for National Health Insurance, Tourism and Hospitality "
        "2015, Solid Waste 2018, Environmental Management Act 2011, "
        "Narcotic Drugs 1993, Bank of Zambia 1996 parent.",
        "",
    ])
