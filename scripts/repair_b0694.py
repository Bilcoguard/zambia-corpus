#!/usr/bin/env python3
"""Repair worker batch b0694 — ZambiaLII SI repair.

Fetches the SI page, locates the source.pdf link, downloads the PDF, and
extracts body text with pdfplumber. Falls back to OCR if extraction is short.
"""
...
DB = "corpus.sqlite"
BATCH = "b0694"
PARSER = "repair-0.6.94"
CERT = "scripts/certs/rapidssl_tls_rsa_ca_g1.pem"
MAX_BATCH = 8
TIME_BUDGET = 18 * 60  # seconds
...
def quality_ok(body: str):
    ...
    # Look for legal text markers
    body_l = body.lower()
    markers = ["section", " act", "regulation", "order", "enacted", "minister", "appointed", "schedule", "by virtue"]
...
def normalise_sections(text: str) -> str:
    # Split run-on section number prefixes — light touch; pdfplumber output usually OK
    # e.g., "1.Citation" -> "1. Citation"; "2.(1)" stays
...
def fetch_pdf(url, dst):
    t0 = time.time()
    # Use curl with rapidssl cert for parliament.gov.zm; for zambialii a plain
    # requests call is fine because Let's Encrypt CA chain is in certifi.
    ...
    try:
        ...
        c.execute("""UPDATE records SET body=?, source_hash=?, fetched_at=?, parser_version=?
                  ...
                  (body, body_hash, time.strftime('%Y-%m-%dT%H:%M:%SZ', time.gmtime()),
                   PARSER, rid))
        # records_fts is FTS5 with content=records — actual schema is
        # (id, title, body, citation, type). Rebuild this row only.
...
def main():
    ...
    c = conn.cursor()
    # virtiofs in this sandbox refuses some sqlite journal-housekeeping syscalls
    # (creates files but EPERMs on fsync/unlink at commit time, producing
    # "disk I/O error"). Switch to in-memory rollback journal with synchronous
    # OFF — durability falls to the post-commit git push, which is the
    # ground-truth backup anyway.
    ...
    log(f"Identified {len(all_targets)} no-body acts/sis (Condition B)")

    # Condition A — corruption check — costly, but quick on 1936 records
