#!/usr/bin/env python3
"""
Batch 0156 — Phase 4 bulk ingestion.

Continuation of the batch 0155 next-tick plan. Probe-only pass —
batch-0155 processed all three surviving probe candidates, so no
SEED block carries forward. ZambiaLII search-API queries rotate to
a fresh set of primary-parent targets flagged unresolved by the
batch-0155 wrap-up: banking and financial services (pre-2017),
mental health (pre-2019), fisheries (pre-2011 principal), forests
(pre-2015 principal), national health insurance (pre-2018),
securities (pre-2016), plus a direct 'patents act' retry query in
case pre-independence ordinances surface, and 'roads and road
traffic' (parent not surfaced by batch-0155's narrower 'road
traffic' probe).

Hits surviving HEAD + title filters fill slots up to
MAX_RECORDS=8. Title filter rejects any slot whose AKN-page title
contains 'amendment', 'appropriation', 'repeal', 'supplementary',
'validation', or 'transitional' — applied pre-write.

Mirrors batch 0155 processor logic (HTML/AKN with PDF fallback +
...
# No seed candidates this batch — batch-0155 processed all three
# surviving probe candidates, so no deferred list carries forward.
SEED_CANDIDATES = []
...
def write_batch_report(records, gaps, total_sections, fetches, integrity_passed,
    ...
    lines = [
        ...
        "## Strategy",
        "",
        "Probe-only pass — batch-0155 processed all three surviving "
        "probe candidates, so no SEED stage runs this batch. Stage 2 "
        "probes the ZambiaLII search API with eight queries targeting "
        "fresh primary-parent targets flagged unresolved by the "
        "batch-0155 wrap-up: banking and financial services, mental "
        "health, fisheries, forests, national health insurance, "
        "securities, plus a direct 'patents act' retry and 'roads "
        "and road traffic' (parent not surfaced by batch-0155's "
        "narrower probe). Hits surviving HEAD + title filters fill "
        f"slots up to MAX_RECORDS={MAX_RECORDS}. Title filter rejects "
        "any slot whose AKN-page title contains `amendment`, "
        "`appropriation`, `repeal`, `supplementary`, `validation`, or "
        "`transitional` — applied pre-write, so rejected slots "
        "produce no raw or record file. PDF fallback is invoked only "
        "when the HTML returns fewer than 2 parsed sections.",
        "",
        "## Committed records",
        "",
    ...
    lines.extend([
        "## Notes",
        "",
        "- No SEED stage this batch — batch-0155 processed all three "
        "surviving probe candidates in that tick, so no deferred "
        "probe queue carries forward.",
        ...
        "- Next tick: continue the primary-statute sweep with the "
        "next rotation of probe keywords — arbitration (primary "
        "2000), copyright (Cap. 406 parent), refugees-control (2017 "
        "parent), higher-education (pre-2013), electoral (pre-2016), "
        "zambia-revenue-authority (1993 parent), and lands-tribunal. "
        "Patents Cap. 400 and Copyright Cap. 406 parents remain "
        "unresolved by all prior probes and may be pre-independence "
        "ordinances not carried on ZambiaLII — consider alphabetical "
        "listing browse as fallback if this tick's direct 'patents "
        "act' probe does not surface them.",
        "",
    ])
