#!/usr/bin/env python3
"""Integrity check for batch-0502 (Phase 5 audit-only zero-yield tick #4).
...
Phase 5 scope, no-regression sweep — matches the b0499 lighter check:
  ...
  - Spot recompute 6 random records (deterministic seed=502) — sha256 must match
...
def main():
    ...
    # spot recompute (seed=502, 6 records)
    if parsed:
        ...
        print(f"spot recompute (seed=502): {spot_ok} / {len(sample)}")
