#!/usr/bin/env python3
"""
Batch 0159 — Phase 4 bulk ingestion.

Probe-only pass. Rotates the ZambiaLII search-API queries to the
next set flagged by batch 0158's next-tick plan:

    hire purchase, stamp duty, juveniles, patents, road traffic,
    immigration, bills of exchange, extradition
...
Mirrors batch 0158 processor logic (HTML/AKN with PDF fallback +
...
No seed candidates this batch — all batch-0158 next-tick primary-
parent targets are probed, not seeded, since prior probe rounds
failed to surface them by keyword. No unconditional repeal-chain
links are pre-declared — the conditional resolver is empty.
"""
...
# No seed candidates this batch — rely entirely on the probe pass.
SEED_CANDIDATES = []
...
def write_batch_report(records, gaps, total_sections, fetches, integrity_passed,
    ...
    lines = [
        ...
        "## Strategy",
        "",
        "Probe-only pass. No seed candidates this batch — all of "
        "the batch-0158 next-tick primary-parent targets are "
        "probed, not seeded, since prior probe rounds failed to "
        "surface them by keyword alone. Stage 2 probes the "
        "ZambiaLII search API with eight fresh rotation queries: "
        "hire purchase, stamp duty, juveniles, patents, road "
        "traffic, immigration, bills of exchange, extradition. "
        "Hits surviving HEAD + title filters fill slots up to "
        f"MAX_RECORDS={MAX_RECORDS}. Title filter rejects any "
        "slot whose AKN-page title contains `amendment` (plus "
        "OCR variants `amendrnent` and `amendement`), "
        ...
        "than 2 parsed sections. No unconditional repeal-chain "
        "links are pre-declared for this batch.",
        "",
        "## Committed records",
        "",
    ...
    lines.extend([
        "## Notes",
        "",
        "- No SEED stage this batch — batch 0158 next-tick "
        "explicitly flagged these as probe rotations rather "
        "than direct year/number seeds, since prior probe "
        "rounds had not surfaced them.",
        ...
        "- Next tick: continue primary-statute sweep with a "
        "fresh rotation of probe keywords — sale of goods, "
        "juveniles (Cap. 53) if still not surfaced, hire "
        "purchase if still not surfaced, stamp duty if still "
        "not surfaced, Patents Cap. 400 via alphabetical "
        "listing fallback if keyword probes continue to "
        "return only amendment variants, Copyright Cap. 406 "
        "parent, and any new candidates surfaced but "
