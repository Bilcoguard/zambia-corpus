#!/usr/bin/env python3
"""
Batch 0160 — Phase 4 bulk ingestion.

Probe-only pass. Rotates the ZambiaLII search-API queries to the
next set flagged by batch 0159's next-tick plan:

    trade, sale of goods, customs, exchange control,
    local government, traffic, national assembly, citizenship
...
Mirrors batch 0159 processor logic (HTML/AKN with PDF fallback +
...
No seed candidates this batch — all targets are probed via
keyword rotation. No unconditional repeal-chain links are
pre-declared.
"""
...
def write_batch_report(records, gaps, total_sections, fetches, integrity_passed,
    ...
    lines = [
        ...
        "## Strategy",
        "",
        "Probe-only pass. No seed candidates this batch — all "
        "targets are surfaced via keyword rotation per the "
        "batch-0159 next-tick plan. Stage 2 probes the "
        "ZambiaLII search API with eight fresh rotation queries: "
        "trade, sale of goods, customs, exchange control, local "
        "government, traffic, national assembly, citizenship. "
        "Hits surviving HEAD + title filters fill slots up to "
    ...
    lines.extend([
        "## Notes",
        "",
        "- No SEED stage this batch — batch 0159 next-tick "
        "flagged these as probe rotations rather than direct "
        "year/number seeds, since narrower prior probes had "
        "not surfaced them.",
        ...
        "fresh rotation of probe keywords — mining act, "
        "fisheries, agriculture, marketing, statistics, "
        "companies act, investment, and constitutional "
        "referendum. If probe yields continue to fall, shift "
        "to alphabetical `/akn/zm/act/` listing traversal "
        "for unresolved Cap. parents.",
        "",
    ])
