#!/usr/bin/env python3
"""
Batch 0154 — Phase 4 bulk ingestion.

Continuation of the batch 0153 next-tick plan. Two-stage candidate
generation:

1. SEED (known year/number) — the 5 candidates that batch 0153's
   probes surfaced but deferred by the MAX_RECORDS=8 cap (after the
   seed block consumed most of the batch). Workers' Compensation Act
   1963/65, National Council for Construction Act 2003/13, Value
   Added Tax Act 1995/4, Mines and Minerals Act 1995/31, Pension
   Scheme Regulation Act 1996/28. These were title-filtered in the
   batch-0153 probe and verified not-in-HEAD by (year, number) tuple
   lookup; AKN slot identifiers are documented in gaps.md so no
   re-probe is required. (Control of Goods Act 1954/12 was also
   listed as probe-deferred by batch 0153 but is already present in
   HEAD as records/acts/1954/act-zm-1954-012-control-of-goods-act-
   1954.json, so it is omitted from the seed list.)

2. PROBE (ZambiaLII search API) — broader keyword queries targeting
   the remaining unresolved primary parents: Patents Cap. 400
   pre-2016 parent, Copyright Cap. 406 parent, Insurance Cap. 392
   parent, plus Arbitration and Adult Literacy-type primary statutes.
   Any hits surviving HEAD + title filters fill remaining slots up to
   MAX_RECORDS=8.

Mirrors batch 0153 processor logic (HTML/AKN with PDF fallback +
pre-write B-POL-ACT-1 title filter). No repeal-chain links applied
this batch — deferred until parent-Act anchors are confirmed.
"""
...
# Deferred candidates surfaced by the batch-0153 probe but parked by the
# MAX_RECORDS=8 cap. Each tuple: (year, number, slug_hint, source_note)
SEED_CANDIDATES = [
    (1963, 65, "workers-compensation", "probe-deferred batch 0153"),
    (2003, 13, "national-council-for-construction", "probe-deferred batch 0153"),
    (1995, 4, "value-added-tax", "probe-deferred batch 0153"),
    (1995, 31, "mines-and-minerals", "probe-deferred batch 0153"),
    (1996, 28, "pension-scheme-regulation", "probe-deferred batch 0153"),
]
...
def write_batch_report(records, gaps, total_sections, fetches, integrity_passed,
    ...
    lines = [
        ...
        "## Strategy",
        "",
        "Two-stage candidate generation. Stage 1 processes the 5 "
        "MAX_RECORDS-deferred candidates that batch 0153's probe "
        "flagged in `gaps.md` (year/number already verified in that "
        "probe): Workers' Compensation 1963/65, National Council for "
        "Construction 2003/13, Value Added Tax 1995/4, Mines and "
        "Minerals 1995/31, Pension Scheme Regulation 1996/28. "
        "(Control of Goods 1954/12 was also on the batch-0153 "
        "deferred list but is already present in HEAD, so it is "
        "omitted.) Stage 2 probes the ZambiaLII search API with six "
        "broader queries (arbitration, adult literacy, cotton, water "
        "resources management, patents 1958, insurance); hits "
        "surviving HEAD + title filters fill remaining slots up to "
    ...
    lines.extend([
        "## Notes",
        "",
        "- Seed candidates are primary statutes that batch 0151/0152 "
        "title-filtered in prior probes and verified as not-in-HEAD by "
        "(year, number) tuple lookup; no re-probe was required.",
        ...
        "- Next tick: continue the primary-statute sweep — "
        "Juveniles Act parent (Cap. 53), Hire Purchase Act parent, "
        "Stamp Duty parent, Sale of Goods parent, Bills of Exchange "
        "parent, Insurance Cap. 392 parent (if not surfaced by this "
        "probe); Patents Cap. 400 and Copyright Cap. 406 parents "
        "remain unresolved by batches 0146, 0152 and 0153 probes.",
        "",
    ])
