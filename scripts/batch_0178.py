#!/usr/bin/env python3
"""
Batch 0178 — Phase 4 sis_corporate: ingest 3 strongest corporate SIs from
batch-0177 pages 9-10 discovery, plus continue /legislation/subsidiary
listing sweep (pages 11 and 12) to surface more corporate candidates.

Ingest targets (from batch 0177 pages 9-10 discovery, strongest first):
  1. si/2015/87  Diplomatic Immunities and Privileges (ZEP-RE PTA
                    Re-Insurance Company) Order, 2015
                    (corporate entity; re-insurance)
  2. si/2014/56  Income Tax (European Investment Bank) (Approval and
                    Exemption) Order, 2014 (bank; investment — corporate)
  3. si/2016/18  Insurance Premium Levy (Exemption) Order, 2016
                    (insurance — corporate/regulatory)

Deferred (marginal): si/2015/33 Road Traffic (Certificates of Security
and Insurance) Displacement — compulsory-insurance keyword match but
not corporate substance.

All 3 targets confirmed novel vs records/sis/ HEAD (no YYYY-NNN prefix
collision under git ls-tree HEAD + on-disk records/sis/YYYY listing).

Additional discovery: fetch /legislation/subsidiary?page=11 and page=12
for next-tick corporate-keyword candidate surfacing.
...
Wall-clock guard: ~10 minutes of processing inside the 20-min tick
(3 candidates × ~90s each + 2 discovery × 6s).
...
# Ingest candidates selected from batch-0177 pages 9-10 discovery
CANDIDATES = [
    (2015, 87,
     "Diplomatic Immunities and Privileges (ZEP-RE PTA Re-Insurance Company) Order, 2015",
     "diplomatic-immunities-and-privileges-zep-re-pta-re-insurance-company-order-2015",
     "corporate-insurance"),
    (2014, 56,
     ...
     "corporate-banking"),
    (2016, 18,
     "Insurance Premium Levy (Exemption) Order, 2016",
     "insurance-premium-levy-exemption-order-2016",
     "corporate-insurance"),
]

# Additional discovery pages (pages 11 and 12; pages 9-10 were batch 0177)
DISCOVERY_PAGES = [11, 12]
