#!/usr/bin/env python3
"""
Batch 0193 — Phase 4 sis_mining rotation (priority_order item 7).

Rotates from sis_data_protection (15 records as of batch 0192) to
sis_mining per the batch-0192 next-tick plan. HEAD currently has only
1 sis_mining record (si-zm-2020-028 Ndola Lime Remission). Discovery
this tick via ZambiaLII /legislation/subsidiary?alphabet=M plus
parent-act back-references on /akn/zm/act/2008/7 yielded 5 novel
candidates, all from the Mines and Minerals Development Act lineage
(2008 Act 7 -> 2015 Act 11).

Per batch 0181/0182 operational note: sandbox bash-tool has a 45s hard
ceiling. With zambialii.org 5s + 1s margin crawl-delay + per-fetch HTTP
+ PDF parse, each record takes ~8-14s. 5 records therefore sliced into
3 + 2 via --slice=START:END with a _work/batch_0193_state.json
resume checkpoint — same pattern as 0182/0183/0192.

Targets (5 x 2 fetches = 10 AKN HTML + PDF):
  - si/2016/7   Mines and Minerals Development (General) Regulations, 2016
  - si/2010/26  Mines and Minerals Development (Mining Rights and Non-Mining Rights) Order, 2010
  - si/2009/27  Mines and Minerals Development (Mining Rights and Non-Mining Rights) Order, 2009
  - si/2000/19  Mines and Minerals (Environmental) (Exemption) Order, 2000
  - si/2000/18  Mines and Minerals (Royalty) (Remission) Order, 2000
"""
import argparse
import hashlib
import io
...
TARGETS = [
    (2016, 7),    # Mines and Minerals Development (General) Regulations, 2016
    (2010, 26),   # Mines and Minerals Development (Mining Rights and Non-Mining Rights) Order, 2010
    (2009, 27),   # Mines and Minerals Development (Mining Rights and Non-Mining Rights) Order, 2009
    (2000, 19),   # Mines and Minerals (Environmental) (Exemption) Order, 2000
    (2000, 18),   # Mines and Minerals (Royalty) (Remission) Order, 2000
]
...
def load_prior_state():
    """Load prior slice results so that --slice runs accumulate."""
...
def main():
    ...
    ap.add_argument("--slice", default=f"0:{len(TARGETS)}",
                    help="Target slice START:END (e.g. 0:3 then 3:5)")
    args = ap.parse_args()
    ...
    fetch_counter = [0]
    results = []
    deadline = time.time() + 40  # internal 40s soft deadline (bash hard=45s)
    started_at = utc_now()

    for (year, number) in slice_targets:
    ...
    if prior:
        ...
        # avoid duplicates if same slot already processed
    ...
    print(f"\n==> batch {BATCH_NUM} slice {args.slice}: +{len(results)} attempted, "
          f"{ok_count}/{len(merged_results)} total ok, slice_fetches={fetch_counter[0]}, "
          f"cum_fetches={merged_fetches}")
    return 0
