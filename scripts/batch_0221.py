...
SUB_PHASE_TAG = "sis_health"

# Keyword tags used to qualify candidate SI link text from alphabet indexes.
# Batch 0214 rotates to sis_employment per batch-0213 next-tick plan
# (sis_tax sub-yield 1/5 last tick is the 1st low-yield tick; combined with
# documented exhaustion observations this triggers full rotation to
# sis_employment, priority_order item 4).
...
# Occupational Health and Safety Act 2010/36, Apprenticeship Act,
# Pneumoconiosis Compensation, Factories Act, Skills Development Act,
# National Productivity Development Act.
CORPORATE_KEYWORDS = [
    # sis_mining scope: Mines and Minerals Development Act 2015/11,
    # Petroleum (Exploration and Production) Act, NHCC Act Cap.173 (monument
    # declarations), Geological Survey Act, Mining Industry Tribunal Act,
    # Explosives Act (mining-side), Quarrying.
    "mines and minerals",
    "mineral",
    "mining",
    "prospecting",
...
def cmd_ingest(args):
    ...
    for ov in args.override_target or []:
        ...
        sp = parts[4] if len(parts) >= 5 else None
        cand = {"year": y, "number": n, "letter": letter, "text": text}
        if sp:
            cand["sub_phase"] = sp
    ...
    for t in chunk:
        ...
        per_sub_phase = t.get("sub_phase") or SUB_PHASE_TAG
        print(f"\n=== SI {year}/{number:03d}  [{per_sub_phase}] ===")
        status, rec, meta = process_target(
            session, year, number, per_sub_phase, head_ids, head_slots,
            last_fetch_t, fetch_counter)
        print(f"  status: {status}")
        entry = {"year": year, "number": number, "sub_phase": per_sub_phase,
        ...
        if rec is not None:
            ...
            with open("gaps.md", "a") as f:
                f.write(f"- [{utc_now()}] si/{year}/{number:03d} status={status} "
                        ...
                        f"batch={BATCH_NUM} sub_phase={per_sub_phase}\n")
...
def cmd_reuse_discovery(args):
    """Phase A (alt): re-verify robots, re-use prior batch's discovery cache."""
    head_ids, head_slots = load_head()
    print(f"HEAD SI records: {len(head_ids)}; slots: {len(head_slots)}")
    ...
    src_path = args.source_discovery
    print(f"=== reuse discovery cache: {src_path} ===")
    with open(src_path) as f:
        prior = json.load(f)
    prior_cands = prior.get("candidates", [])
    novel = [c for c in prior_cands if (c["year"], c["number"]) not in head_slots]
    novel.sort(key=lambda c: (-c["year"], c["number"]))
    print(f"  prior candidates: {len(prior_cands)}; remaining (not in HEAD): {len(novel)}")
    ...
    state = {
        ...
        "robots_sha256": robots_hash,
        "alphabets": [],
        "novel_total_pre_keyword": len(prior_cands),
        "novel_kept_post_keyword": len(novel),
        "candidates": novel,
        "reused_from": src_path,
    }
    os.makedirs("_work", exist_ok=True)
    with open(f"_work/batch_{BATCH_NUM}_discovery.json", "w") as f:
        json.dump(state, f, indent=2)
    print(f"\nDISCOVERY (reuse) done: {len(novel)} candidates, fetch_end={fetch_counter[0]}")
    for t in novel[:30]:
        print(f"  - SI {t['number']:03d} of {t['year']} (alphabet={t.get('letter','-')}) "
              f"text={t['text'][:80]}")
    return 0
...
def main():
    ...
    a.add_argument("--alphabets", default="MNPQGE")
    ...
    d = sub.add_parser("reuse_discovery")
    d.add_argument("--fetch-start", type=int, required=True)
    d.add_argument("--source-discovery", required=True)
    ...
    if args.cmd == "reuse_discovery":
        return cmd_reuse_discovery(args)
