#!/usr/bin/env python3
"""
Batch 0176 — Phase 4 sis_corporate: ingest 2 known corporate SIs from
batch-0175 discovery, plus continue /legislation/subsidiary listing sweep
(pages 7 and 8) to surface more corporate candidates for next tick.

Ingest targets (from batch 0175 pages 5-6 discovery):
  1. si/2019/62  Income Tax (Konoike Construction Company Limited)
                  (Approval and Exemption) Order, 2019
  2. si/2019/59  Insurance (Fidelity Fund) Regulations, 2019

Both were filtered as corporate-adjacent (keywords "compan", "insur"),
are novel vs HEAD (records/sis/2019/), and reside at
zambialii.org/akn/zm/act/si/2019/NN.

Additional discovery: fetch /legislation/subsidiary?page=7 and page=8 for
next-tick corporate-keyword candidate surfacing.

Filter refinement: CORPORATE_KEYWORDS now uses word-boundary regex to
eliminate the "pension" substring-matching-"suspension" false positive
documented in batch-0175 (see gaps.md).
...
# Ingest candidates selected from batch-0175 page-5 discovery
CANDIDATES = [
    (2019, 62,
     "Income Tax (Konoike Construction Company Limited) (Approval and Exemption) Order, 2019",
     "income-tax-konoike-construction-company-limited-approval-and-exemption-order-2019",
     "corporate-tax-exemption"),
    (2019, 59,
     "Insurance (Fidelity Fund) Regulations, 2019",
     "insurance-fidelity-fund-regulations-2019",
     "corporate-insurance"),
]

# Additional discovery pages (pages 7 and 8; pages 5-6 were batch 0175)
DISCOVERY_PAGES = [7, 8]
...
def main():
    ...
    print(f"[{started_iso}] batch_0176 starting in {WORKSPACE}", flush=True)
