#!/usr/bin/env python3
"""b0667 repair worker — ZambiaLII SI repair (no-body acts/SIs continuation).

Drains the 113 acts/SIs with no body. Uses /tmp staging DB to avoid
virtiofs write-back I/O errors on the workspace volume (95% capacity).

Modeled on repair_b0664.py established pattern. Parser version: repair-0.6.7.
"""
import sqlite3, urllib.request, urllib.error, ssl, re, os, sys, hashlib, json, time, subprocess, shutil
from pathlib import Path
from bs4 import BeautifulSoup

WORKSPACE = Path("/sessions/adoring-funny-faraday/mnt/corpus")
PDF_DIR = Path("/tmp/b0667_pdfs")
...
# Stage on /tmp to avoid virtiofs write-back issues on the 95%-full corpus volume
TMP_DIR = Path("/tmp/b0667_recover")
...
def main():
    ...
    for rec in targets:
        ...
        # delete-then-insert FTS row to refresh
    ...
    Path("/tmp/b0667_recover/result.json").write_text(json.dumps(result, indent=2))
