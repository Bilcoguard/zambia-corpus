"""Batch 0300 — Phase 4 / minimal upstream-refresh tick.

Inherited closing state (after b0290..b0299):
  ...
  - OCR backlog: 21 (unchanged from b0293..b0299).
...
PARSER_VERSION: 0.5.0 (probe-only — same as b0290..b0299)
...
PROBE RESULTS (see _work/batch_0300_probe.json for full data):
...
SANDBOX NOTE (carried from b0299):

  - parliament.gov.zm does not send the RapidSSL TLS RSA CA G1
    intermediate in its TLS chain. The previous worker (host-side) had
    the intermediate in its system trust store; the corpus-worker
    sandbox does not. Worked around by appending
    ...
    used (we still verify the chain — we just supplement the trust
    store with the published RapidSSL intermediate). This preserves the
    Non-negotiable #3 (respect sources) and #1 (no fabrication).
"""
...
Phase 4 disposition unchanged from b0293..b0299: cannot meaningfully
