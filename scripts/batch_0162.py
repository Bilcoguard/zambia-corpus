#!/usr/bin/env python3
"""
Batch 0162 — Phase 4 bulk ingestion.

Probe-only pass. Rotates the ZambiaLII search-API queries to the
next set flagged by batch 0161's next-tick plan:

    forestry, veterinary, animal health, education,
    legal practitioners, public health, food and drugs, land survey
...
Mirrors batch 0161 processor logic (HTML/AKN with PDF fallback +
...
def write_batch_report(records, gaps, total_sections, fetches, integrity_passed,
    ...
    lines = [
        ...
        "batch-0161 next-tick plan. Stage 2 probes the "
        "ZambiaLII search API with eight fresh rotation queries: "
        "forestry, veterinary, animal health, education, "
        "legal practitioners, public health, food and drugs, "
        "land survey. "
        "Hits surviving HEAD + title filters fill slots up to "
    ...
    lines.extend([
        "## Notes",
        "",
        "- No SEED stage this batch — batch 0161 next-tick "
        ...
        "narrower probe keywords — water resources, wildlife, "
        "national parks, insurance, pensions, securities, "
        "telecommunications, energy regulation.",
        "",
    ])
