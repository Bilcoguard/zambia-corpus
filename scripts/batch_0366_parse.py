#!/usr/bin/env python3
"""Batch 0366 parser-only step (REPARSE-FIRST PASS, continuation of b0365).
...
Picks up after b0365's 2024/{3,2,1}+2023/{27,26,25,24,23} slice
with the next eight raw-on-disk candidates from the 2023 backlog:
  zmcc 2023/21, 2023/20, 2023/19, 2023/18, 2023/17, 2023/16,
  2023/15, 2023/14.

(zmcc/2023/17 has only the raw HTML on disk — the PDF was never
 fetched in earlier batches; the parser will defer it with
 reason="raw bytes not on disk" via the missing-file guard. The
 other seven have full HTML+PDF pairs.)
...
TARGETS = [
    ...
    # before locking the slice — see worker.log for b0365).
    # Batch 0366 (REPARSE PASS): continuation of b0365 deferred queue,
    # year-DESC then num-DESC. All raw HTML (and 7-of-8 PDFs) already
    # on disk from earlier batches; this run consumes zero fetch budget.
    # Dates verified via grep of canonical /eng@YYYY-MM-DD link in each
    # raw HTML before locking the slice (worker.log b0366).
    ("zmcc", 2023, 21, "2023-10-27"),
    ("zmcc", 2023, 20, "2023-10-26"),
    ("zmcc", 2023, 19, "2023-10-26"),
    ("zmcc", 2023, 18, "2023-10-02"),
    ("zmcc", 2023, 17, "2023-03-09"),
    ("zmcc", 2023, 16, "2023-07-11"),
    ("zmcc", 2023, 15, "2023-10-26"),
    ("zmcc", 2023, 14, "2023-03-10"),
]
