#!/usr/bin/env python3
"""Batch 0372 parser-only step (REPARSE-FIRST PASS, continuation of b0371).

Re-parses 3 ZMCC 2021 raw HTML+PDF pairs already on disk that were
...
Slice: continues the 2021 ZMCC raw-on-disk no-record backlog. b0371
took {2022/7, 2022/5, 2022/4, 2022/3, 2022/2, 2022/1, 2021/22, 2021/21}
...
re-parsed under v0.3.1 + judges_no_comma guard:
  zmcc 2021/19, 2021/18, 2021/12.
Excluded from the slice: zmcc/2021/{15, 14} are already classified
`pdf_extraction_empty_likely_scanned` (parser cannot help — needs
OCR pass per b0371 recommendation #2); zmcc/2021/21 was just
attempted in b0371 (deferred html_no_summary_pdf_no_match) and
re-running v0.3.1 would yield an identical deferral — skipped to
avoid redundant work.

Slice size 3 (under MAX_BATCH_SIZE=8): the 2021 ZMCC backlog has
only 3 v0.3.1-amenable candidates remaining; we do not pad the
batch with already-classified deferreds. Future v0.3.2 vocabulary
widening will reopen the 2022/2024/2025 ZMCC backlog (subject to
Peter's approval per BRIEF.md non-negotiable on parser changes).

All three have full HTML+PDF pairs on disk; expression dates were
...
TARGETS = [
    ...
    # before locking the slice — see worker.log for b0372).
    # Batch 0372 (REPARSE PASS): continues 2021 ZMCC backlog after b0371.
    # All raw HTML+PDF pairs already on disk from earlier batches;
    # this run consumes zero fetch budget.
    ("zmcc", 2021, 19, "2021-03-25"),
    ("zmcc", 2021, 18, "2021-08-18"),
    ("zmcc", 2021, 12, "2021-06-30"),
]
