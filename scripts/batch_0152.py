#!/usr/bin/env python3
"""
Batch 0152 — Phase 4 bulk ingestion.

Continuation of the batch 0151 next-tick plan. Probes the ZambiaLII
search API for primary-parent Acts in the Patents / Copyright /
Chartered Institute / Tobacco / Co-operative Societies / Property
Transfer Tax / Carriage by Air / Loans and Guarantees families
that are not yet in HEAD. Mirrors batch 0151 processor logic
(probe-and-ingest + pre-write B-POL-ACT-1 title filter + PDF
fallback when HTML sparse).

No repeal-chain links applied this batch — deferred until the
Insurance Act Cap.392 parent and the Patents Cap.400 parent are
confirmed, so that any repealed_by linkage can be anchored on
documented parents rather than provisional assumptions.
"""
...
def write_batch_report(records, gaps, total_sections, fetches, integrity_passed,
    ...
    lines = [
        ...
        "## Strategy",
        "",
        "Continuation of the batch 0151 next-tick plan — primary-statute "
        "sweep for Patents (Cap. 400 parent), Copyright and Performance "
        "Rights (Cap. 406 parent), Chartered Institute of Public Relations, "
        "Tobacco, Co-operative Societies, Property Transfer Tax, Carriage "
        "by Air, and Loans and Guarantees families. Candidate (year, "
    ...
    if repeal_changes:
        ...
        lines.append(
            "No new repeal-chain links applied this batch — deferred until "
            "the Insurance Act Cap.392 parent and the Patents Act Cap.400 "
            "parent are confirmed."
        )
    lines.extend([
        "",
        "## Probe summary",
    ...
    lines.extend([
        ...
        "queries on Sale of Goods, Bills of Exchange, Juveniles Act parent "
        "(Cap. 53), Hire Purchase Act, Stamp Duty Act parent, and Insurance "
        "Act Cap. 392 parent (likely 1997); also revisit any deferred "
        "candidates from the batch cap.",
        "",
    ])
