#!/usr/bin/env python3
"""b0664 repair worker — ZambiaLII SI repair (no-body acts/SIs continuation).

Drains the 121 acts/SIs with no body. Uses /tmp staging DB to avoid
virtiofs write-back I/O errors on the workspace volume.
"""
import sqlite3, urllib.request, urllib.error, ssl, re, os, sys, hashlib, json, time, subprocess, shutil
from pathlib import Path
from bs4 import BeautifulSoup

WORKSPACE = Path("/sessions/great-funny-noether/mnt/corpus")
PDF_DIR = WORKSPACE / "_repair_b0664_pdfs"
...
TMP_DIR = Path("/tmp/b0664_recover")
...
def main():
    ...
    Path("/tmp/b0664_recover/result.json").write_text(json.dumps(result, indent=2))
