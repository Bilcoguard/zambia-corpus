#!/usr/bin/env python3
"""Batch 0369 parser-only step (REPARSE-FIRST PASS, continuation of b0368).
...
Slice: continuation of the 2022 ZMCC backlog. b0368 took DESC slice
{34,33,32,31,30,27,25,24} (0 written / 8 deferred — all 2022 HTML
uses space-separated judges plus non-standard summary verbs).
This batch takes the next eight raw-on-disk no-record candidates
(year-DESC then num-DESC), per b0368 next-tick recommendation:
  zmcc 2022/23, 2022/22, 2022/21, 2022/20, 2022/18, 2022/17,
  2022/16, 2022/15.
...
TARGETS = [
    ...
    # before locking the slice — see worker.log for b0369).
    # Batch 0369 (REPARSE PASS): continuation of b0368 2022 ZMCC sweep.
    # b0368 took {34,33,32,31,30,27,25,24} (0 written / 8 deferred).
    # This batch takes the next eight raw-on-disk no-record candidates
    # (year-DESC then num-DESC) per b0368 next-tick plan.
    # All raw HTML+PDF pairs already on disk from earlier batches;
    # this run consumes zero fetch budget.
    ("zmcc", 2022, 23, "2022-10-17"),
    ("zmcc", 2022, 22, "2022-09-23"),
    ("zmcc", 2022, 21, "2022-09-29"),
    ("zmcc", 2022, 20, "2022-09-21"),
    ("zmcc", 2022, 18, "2022-09-07"),
    ("zmcc", 2022, 17, "2022-08-31"),
    ("zmcc", 2022, 16, "2022-08-25"),
    ("zmcc", 2022, 15, "2022-07-29"),
]
