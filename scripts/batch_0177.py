#!/usr/bin/env python3
"""
Batch 0177 — Phase 4 sis_corporate: ingest 4 strongest corporate SIs from
batch-0176 pages 7-8 discovery, plus continue /legislation/subsidiary
listing sweep (pages 9 and 10) to surface more corporate candidates.

Ingest targets (from batch 0176 pages 7-8 discovery, strongest first):
  1. si/2017/19  National Pension Scheme (Investment) Regulations, 2017
                    (pension, investment — corporate governance)
  2. si/2016/9   Financial Intelligence Centre (General) Regulations, 2016
                    (AML/CFT cornerstone; financial intelligence)
  3. si/2016/52  Financial Intelligence Centre (Prescribed Thresholds)
                  Regulations, 2016 (AML/CFT thresholds)
  4. si/2016/95  National Pension Scheme (Medical Board) Regulations, 2016
                    (pension — corporate-adjacent)

All 4 confirmed novel vs records/sis/ HEAD (no YYYY-NNN prefix collision).
Deferred to a later tick (per batch-0176 next-tick plan): 2017/70, 2017/42
(narrow company-specific Income Tax exemption orders).

Additional discovery: fetch /legislation/subsidiary?page=9 and page=10 for
...
Wall-clock guard: ~12 minutes of processing inside the 20-min tick
(4 candidates × ~90s each + 2 discovery × 6s).
...
# Word-boundary patterns: avoids "pension" matching "suspension".
# Each token must be a standalone word (optionally with a hyphen or apostrophe).
CORPORATE_KEYWORDS = [
    r"compan(?:y|ies)", r"corporate", r"pacra", r"patent", r"trademark",
    r"trade mark", r"intellectual property", r"bank", r"financial",
    r"securities", r"stock exchange", r"capital market", r"insur\w*",
    r"pension", r"procurement", r"anti-money", r"money launder",
    r"financial intelligence", r"competition", r"consumer protection",
    r"investment",
]
CORPORATE_RE = re.compile(
    r"\b(?:" + "|".join(CORPORATE_KEYWORDS) + r")\b",
    re.IGNORECASE,
)

# Ingest candidates selected from batch-0176 pages 7-8 discovery
CANDIDATES = [
    (2017, 19,
     "National Pension Scheme (Investment) Regulations, 2017",
     "national-pension-scheme-investment-regulations-2017",
     "corporate-pensions"),
    (2016, 9,
     ...
     "corporate-amlcft"),
    (2016, 52,
     "Financial Intelligence Centre (Prescribed Thresholds) Regulations, 2016",
     "financial-intelligence-centre-prescribed-thresholds-regulations-2016",
     "corporate-amlcft"),
    (2016, 95,
     "National Pension Scheme (Medical Board) Regulations, 2016",
     "national-pension-scheme-medical-board-regulations-2016",
     "corporate-pensions"),
]

# Additional discovery pages (pages 9 and 10; pages 7-8 were batch 0176)
DISCOVERY_PAGES = [9, 10]
...
def discover_page(page_num):
    ...
    for y, n, t, h in listings:
        if (y, n) not in seen:
            seen[(y, n)] = (t, h)
    # JSON-serializable form of seen (tuple keys -> list of [[y,n],[t,h]])
    seen_list = [[[y, n], [t, h]] for (y, n), (t, h) in seen.items()]
    return {
        "page": page_num,
        "raw_path": raw_path,
        "sha256": r["sha256"],
...
def main():
    ...
    print(f"[{started_iso}] batch_{BATCH_NUM} starting in {WORKSPACE}", flush=True)

    # Sub-tick mode: CLI arg selects phase.
    # python3 scripts/batch_0177.py CAND 0     -> process CANDIDATES[0]
    # python3 scripts/batch_0177.py DISC 9     -> discovery page 9
    # python3 scripts/batch_0177.py FINALIZE   -> emit final state w/ novel-corporate analysis
    ...
    # Keep original fetch-count scheme but add cumulative across sub-ticks
    global fetch_count
    if mode == "CAND":
        idx = int(arg)
        if idx >= len(CANDIDATES):
            print(f"  idx {idx} out of range")
            return
        year, number, title_hint, slug_hint, theme = CANDIDATES[idx]
        # Skip if already committed
        for c in state["committed"]:
            if c.get("year") == year and c.get("number") == number:
                print(f"  already committed si/{year}/{number}, skipping")
                return
        for g in state["gapped"]:
            if g.get("year") == year and g.get("number") == number:
                print(f"  already gapped si/{year}/{number}, skipping")
                return
        try:
        ...
        if err:
            ...
            state["committed"].append({
                ...
                "record_path": os.path.join(
                    RECORDS_DIR, str(year), f"{record['id']}.json"),
        ...
        state["fetch_count_cum"] += fetch_count
        save_state(state)
        print(f"  Sub-tick done. Cumulative fetches: {state['fetch_count_cum']}")
        return

    if mode == "DISC":
        page = int(arg)
        # Skip if already fetched this page
        for d in state["discovery"]:
            if d.get("page") == page:
                print(f"  discovery page {page} already fetched, skipping")
                return
        try:
            d = discover_page(page)
        except Exception as e:
            import traceback
            traceback.print_exc()
            d = {"page": page, "error": f"exception: {e}"}
        # Persist 'seen' in state for FINALIZE novel-corporate analysis
        state["discovery"].append(d)
        state["fetch_count_cum"] += fetch_count
        save_state(state)
        print(f"  Sub-tick done. Cumulative fetches: {state['fetch_count_cum']}")
        return
    ...
    if mode == "FINALIZE":
        ...
        for d in state["discovery"]:
            if "seen" not in d:
                continue
            # seen was written as dict with tuple keys -> serialized as list of [[y,n],[t,h]]
            seen_iter = d["seen"]
            if isinstance(seen_iter, dict):
                items = seen_iter.items()
            else:
                items = seen_iter
            for entry in items:
                if isinstance(entry, tuple):
                    ...
                    (y, n), (t, h) = entry[0], entry[1]
                if (int(y), int(n)) in existing:
                    continue
                matched = CORPORATE_RE.findall(t)
                item = {
                    "year": int(y), "num": int(n), "title": t, "href": h,
                    "page": d.get("page"),
                    "matched_keywords": sorted({m.lower() for m in matched}),
                }
                if matched:
                    all_novel_corporate.append(item)
                else:
                    all_novel_other.append(item)
        state["completed_at"] = utc_now_iso()
        state["novel_corporate_candidates"] = all_novel_corporate
        state["novel_other_candidates_count"] = len(all_novel_other)
        # Strip 'seen' from discovery to avoid JSON bloat
        for d in state["discovery"]:
            d.pop("seen", None)
        save_state(state)
        print(
            f"FINALIZE done. Committed={len(state['committed'])} "
            f"Gapped={len(state['gapped'])} "
            f"Discovery={len(state['discovery'])} "
            f"NovelCorp={len(all_novel_corporate)} "
            f"NovelOther={len(all_novel_other)}"
        )
        return

    # LEGACY (all in one) — not used for sub-tick splitting
    print("  LEGACY mode — running full pipeline (may exceed sandbox budget)", flush=True)
        ...
        if "seen" in d:
            for (year, num), (title, href) in d["seen"].items():
                if (year, num) in existing:
                    continue
                # word-boundary regex avoids "pension" in "suspension"
                matched = CORPORATE_RE.findall(title)
                entry = {
