"""Batch 0330 — Phase 4 / minimal upstream-refresh tick (41st consecutive steady-state tick).

Inherited closing state (after b0290..b0329):
  ...
  - OCR backlog: 21 (unchanged from b0293..b0329).
  - Cumulative records: acts 1169, SIs 539, judgments 25 (all unchanged
    from b0311..b0329).
  - 5 known dual-location act records (flat + per-year copy of the same
    JSON; same `id`) — long-standing pre-tick state, not a regression
    introduced this tick. Tracked separately; outside this probe's
    integrity scope.
...
PARSER_VERSION: 0.5.0 (probe-only — same as b0290..b0329)
...
PROBE RESULTS (see _work/batch_0330_probe.json for full data):
  ...
  /legislation/recent:    15 act links (2026/1..11 + 2025/6..9).
                          ...
                          (UNCHANGED vs b0329 — feed surface and
                          ranking are byte-for-byte identical to
                          b0329).
  parliament page 0:      20 act links (2026/1..11 + 2025/21..29).
                          ALL 20 in corpus.
                          page bytes: 38,208 (unchanged across all 41
                          steady-state ticks).
                          page sha256: 2ebf59d409ff2825f160eefd8329e222f695245afe2eacd934c8e325ec228024
...
SANDBOX NOTE (carried from b0299..b0329):
...
# The probe lives at _work/batch_0330_probe.py (gitignored scratch).
PICKS = []
"""
...
Phase 4 disposition unchanged from b0293..b0329: cannot meaningfully
