#!/usr/bin/env python3
"""Repair worker batch 003 — fixes up to 8 corrupted Act/SI bodies.
...
WORKDIR = "/sessions/charming-dazzling-edison/mnt/corpus"
PDF_DIR = "/tmp/repair-batch-003-pdfs"
...
def main():
    ...
    tmp_root = tempfile.mkdtemp(prefix="repair_batch_")
    ...
    for i, (rid, url) in enumerate(queue):
        ...
        if len(text) < 200:
            ...
            with open(LOG, "a") as f:
                f.write(f"{now_iso()}\trepair-batch-003\tocrmypdf not available - deferred {rid}\n")
    ...
    conn.close()
    if integrity_ok and successes > 0:
    ...
    # Persist run log
    with open(os.path.join(WORKDIR, "_repair_batch_003_result.json"), "w") as f:
